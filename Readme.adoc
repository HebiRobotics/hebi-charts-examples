...
Some constructor calls for UI elements may block until the next frame for proper initialization. Subsequent calls are non-blocking. For example, a random walk in all languages could look like below,

.Random Walk Example
image::docs/images/python_randomwalk.png[align="center"]
...
=== StreamView
Low-latency image viewer that reads from shared memory buffers. Requires HEBI's https://docs.hebi.us/downloads_changelogs.html#software[Video Acquisition] tools. The format is non-final, so expect potential changes.
...
.Latency Example
image::docs/images/python_latency.png[align="center"]
