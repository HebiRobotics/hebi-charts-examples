...
HEBI Charts is a 2D and 3D visualization library designed for low-overhead rendering of high-speed telemetry with a focus on the robotics domain.

image::docs/images/main_snapshot.png[align="center"]

It features a stable C ABI supporting Linux, macOS, and Windows, with idiomatic bindings for Python, MATLAB, and C++.
...
The API is purely polling-based to integrate seamlessly with linear control loops. Event checks like `button.was_pressed()` use an atomic read-and-reset mechanism that guarantees that UI events are caught.
...
Note that on Windows, `LoopTimer` automatically manages the system timer resolution via `timeBeginPeriod(1)` and `timeEndPeriod()`. This ensures that `sleep` operations use a 1ms resolution rather than the 15.6ms default.
