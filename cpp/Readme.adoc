...
HEBI Charts provides a zero-configuration setup via link:hebi_charts/hebi_charts.cmake[hebi_charts.cmake]. Including this CMake file automatically downloads the required platform-specific binaries and stores them in a shared local system cache.

The included `target_add_hebi_charts` macro adds include paths and automatically copies the necessary dynamic library into the build output directory for correct runtime resolution.
