...
// mxGetScalar is undefined on an empty array and unpredictable on a struct
// or cell, so only classes it is actually defined for get through.
inline bool isNumericScalar(const mxArray* arg) {
    return (mxIsNumeric(arg) || mxIsLogical(arg) || mxIsChar(arg))
            && !mxIsComplex(arg) && mxGetNumberOfElements(arg) == 1;
}

template <typename T>
inline T getScalar(const mxArray* arg, const char* name) {
    if (!isNumericScalar(arg)) {
        mexErrMsgIdAndTxt("hebi_charts:TypeError", "Argument '%s' must be a numeric scalar.", name);
    }
    // mxGetScalar goes through a double, which loses 64 bit integers above 2^53
    switch (mxGetClassID(arg)) {
        case mxINT64_CLASS: return static_cast<T>(((const int64_t*)mxGetData(arg))[0]);
        case mxUINT64_CLASS: return static_cast<T>(((const uint64_t*)mxGetData(arg))[0]);
        default: return static_cast<T>(mxGetScalar(arg));
    }
}

// Only the generated wrappers can reach the gateway, so this guards against a generator
// bug rather than user error. Every case indexes prhs directly.
inline void checkArgCount(int nrhs, int expected, const char* name) {
    if (nrhs - 1 != expected) {
        mexErrMsgIdAndTxt("hebi_charts:InvalidInput", "'%s' expects %d argument(s), but received %d.",
                          name, expected, nrhs - 1);
    }
}

// Struct for getting error return information. Every call that can throw fills it.
// The char pointers are only valid until the current thread calls any other lib function.
struct ErrorInfo {
    const char* message;
    const char* id;
    int32_t code;
...
// mexErrMsgIdAndTxt immediately reports an error and jumps over the rest of the function.
inline void checkError(const ErrorInfo& error) {
    if (error.code != 0) {
        mexErrMsgIdAndTxt(error.id != nullptr ? error.id : "hebi_charts:Error",
                          "%s", error.message != nullptr ? error.message : "unknown error");
    }
}
...
struct MatlabStringArray {
    ...
    operator char**() const { return ptrs; }
...
// Layout-compatible with the by-value DoubleSpan struct in the C ABI
struct DoubleSpan {
    const double* data;
    size_t length;
};

// Returned spans are library-owned and only valid until the next call,
// so they get copied immediately
inline mxArray* createMexDoubleArray(DoubleSpan span) {
    if (!span.data || span.length == 0) {
        return createEmpty();
    }
    mxArray* array = mxCreateDoubleMatrix(1, span.length, mxREAL);
    std::memcpy(mxGetPr(array), span.data, span.length * sizeof(double));
    return array;
}

// Layout-compatible with the by-value Transform4x4 struct in the C ABI
struct Transform4x4 {
    const double* data;
    int32_t ordering;
};

// MATLAB char data is UTF-16, and the Matrix API has no UTF-8 aware way to build it.
// mxCreateString decodes in MATLAB's default character set, which its own header describes
// as "either UTF-8 encoding or the local codepage encoding". The UTF-8 default was added in
// R2020a, but we want to support back down to R2018b. Counting the utf8 code units keeps
...
inline mxArray* createMexString(const char* c_str) {
    ...
    // Continuation bytes never start a character, a four byte sequence needs a surrogate pair
    const auto* utf8 = reinterpret_cast<const unsigned char*>(c_str);
    mwSize units = 0;
    for (const unsigned char* b = utf8; *b; ++b) {
        if ((*b & 0xC0) != 0x80) units += (*b >= 0xF0) ? 2 : 1;
    }

    const mwSize dims[2] = {1, units};
    mxArray* array = mxCreateCharArray(2, dims);
    auto* out = static_cast<mxChar*>(mxGetData(array));
    const mxChar* const end = out + units; // malformed input must not go beyond the count

    for (const unsigned char* b = utf8; *b && out < end; ) {
        uint32_t cp = *b++;
        if (cp >= 0x80) {
            // Mask the length bits off the lead byte, then fold in what follows
            cp &= (cp >= 0xF0) ? 0x07 : (cp >= 0xE0) ? 0x0F : 0x1F;
            while ((*b & 0xC0) == 0x80) cp = (cp << 6) | (*b++ & 0x3F);
        }

        if (cp <= 0xFFFF) {
            *out++ = static_cast<mxChar>(cp);
        } else {
            cp -= 0x10000; // outside the BMP, so it takes a surrogate pair
            *out++ = static_cast<mxChar>(0xD800 | (cp >> 10));
            if (out < end) *out++ = static_cast<mxChar>(0xDC00 | (cp & 0x3FF));
        }
    }
    return array;
}
...
inline mxArray* createMexScalar(T value) {
    ...
    // create issues. This gives us 2^53 bits for integers before loss of precision,
    // which is fine for everything up to 32 bit. Wider types get their own class below.
    return mxCreateDoubleScalar(static_cast<double>(value));
}

// 2^53, the largest integer a double holds exactly
static const int64_t MAX_EXACT_DOUBLE_INT = 9007199254740992LL;

// 64 bit integers stay double while they fit the mantissa
inline mxArray* createMexInt64(int64_t value) {
    if (value <= MAX_EXACT_DOUBLE_INT && value >= -MAX_EXACT_DOUBLE_INT) {
        return mxCreateDoubleScalar(static_cast<double>(value));
    }
    mxArray* array = mxCreateNumericMatrix(1, 1, mxINT64_CLASS, mxREAL);
    ((int64_t*)mxGetData(array))[0] = value;
    return array;
}

inline mxArray* createMexSize(size_t value) {
    if ((uint64_t) value <= (uint64_t) MAX_EXACT_DOUBLE_INT) {
        return mxCreateDoubleScalar(static_cast<double>(value));
    }
    mxArray* array = mxCreateNumericMatrix(1, 1, mxUINT64_CLASS, mxREAL);
    ((uint64_t*)mxGetData(array))[0] = (uint64_t) value;
    return array;
}
...
MEX_EXPORT void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    switch (funcHash) {
        ...
        case /* hash32_fnv1a("hebi_charts_Camera_setView") */ 3801818395: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Camera_setView");
            int view = getScalar<int>(prhs[2], "view");
        ...
        case /* hash32_fnv1a("hebi_charts_Camera_reset") */ 1415684501: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Camera_reset");
            hebi_charts_Camera_reset(getPtr<void*>(prhs[1]));
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Camera_applyRotation") */ 3985810136: {
            static auto hebi_charts_Camera_applyRotation = libLoader.getFunc<void(*)(ErrorInfo*, void*, double, double, double, double)>("hebi_charts_Camera_applyRotation");
            checkArgCount(nrhs, 5, "hebi_charts_Camera_applyRotation");
            ErrorInfo _error = {};
            double qx = getScalar<double>(prhs[2], "qx");
            double qy = getScalar<double>(prhs[3], "qy");
            double qz = getScalar<double>(prhs[4], "qz");
            double qw = getScalar<double>(prhs[5], "qw");
            hebi_charts_Camera_applyRotation(&_error, getPtr<void*>(prhs[1]), qx, qy, qz, qw);
            checkError(_error);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Camera_setDistance") */ 1243202457: {
            static auto hebi_charts_Camera_setDistance = libLoader.getFunc<void(*)(ErrorInfo*, void*, double)>("hebi_charts_Camera_setDistance");
            checkArgCount(nrhs, 2, "hebi_charts_Camera_setDistance");
            ErrorInfo _error = {};
            double distanceInMeters = getScalar<double>(prhs[2], "distanceInMeters");
            hebi_charts_Camera_setDistance(&_error, getPtr<void*>(prhs[1]), distanceInMeters);
            checkError(_error);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Camera_setPan") */ 2602784433: {
            static auto hebi_charts_Camera_setPan = libLoader.getFunc<void(*)(ErrorInfo*, void*, double, double, double)>("hebi_charts_Camera_setPan");
            checkArgCount(nrhs, 4, "hebi_charts_Camera_setPan");
            ErrorInfo _error = {};
            double x = getScalar<double>(prhs[2], "x");
            double y = getScalar<double>(prhs[3], "y");
            double z = getScalar<double>(prhs[4], "z");
            hebi_charts_Camera_setPan(&_error, getPtr<void*>(prhs[1]), x, y, z);
            checkError(_error);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Camera_setControlsVisible") */ 3987743262: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Camera_setControlsVisible");
            int value = getScalar<int>(prhs[2], "value");
        ...
        case /* hash32_fnv1a("hebi_charts_Camera_release") */ 2037833667: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Camera_release");
        ...
        case /* hash32_fnv1a("hebi_charts_Control_isEnabled") */ 3777718777: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Control_isEnabled");
        ...
        case /* hash32_fnv1a("hebi_charts_Control_setEnabled") */ 3290542769: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Control_setEnabled");
            int enabled = getScalar<int>(prhs[2], "enabled");
        ...
        case /* hash32_fnv1a("hebi_charts_Control_getLabel") */ 4292032106: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Control_getLabel");
        ...
        case /* hash32_fnv1a("hebi_charts_Control_setLabel") */ 2485366894: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Control_setLabel");
        ...
        case /* hash32_fnv1a("hebi_charts_Control_getTooltip") */ 1933368807: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Control_getTooltip");
        ...
        case /* hash32_fnv1a("hebi_charts_Control_setTooltip") */ 871102075: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Control_setTooltip");
        ...
        case /* hash32_fnv1a("hebi_charts_Control_isVisible") */ 633637578: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Control_isVisible");
        ...
        case /* hash32_fnv1a("hebi_charts_Control_setVisible") */ 264986930: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Control_setVisible");
            int visible = getScalar<int>(prhs[2], "visible");
        ...
        case /* hash32_fnv1a("hebi_charts_Control_release") */ 795521629: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Control_release");
        ...
        case /* hash32_fnv1a("hebi_charts_Button_getText") */ 236848194: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Button_getText");
        ...
        case /* hash32_fnv1a("hebi_charts_Button_setText") */ 578248390: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Button_setText");
        ...
        case /* hash32_fnv1a("hebi_charts_Button_isPressed") */ 3638190771: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Button_isPressed");
        ...
        case /* hash32_fnv1a("hebi_charts_Button_wasPressed") */ 2864445990: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Button_wasPressed");
        ...
        case /* hash32_fnv1a("hebi_charts_Button_to_Control") */ 3682425080: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Button_to_Control");
            void* _value = hebi_charts_Button_to_Control(getPtr<void*>(prhs[1]));
        ...
        case /* hash32_fnv1a("hebi_charts_Dropdown_setOptions") */ 1446832170: {
            ...
            checkArgCount(nrhs, 3, "hebi_charts_Dropdown_setOptions");
            MatlabStringArray options(prhs[2]);
            size_t count = getScalar<size_t>(prhs[3], "count");
        ...
        case /* hash32_fnv1a("hebi_charts_Dropdown_getSelectedIndex") */ 2533218167: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Dropdown_getSelectedIndex");
        ...
        case /* hash32_fnv1a("hebi_charts_Dropdown_setSelectedIndex") */ 1323411971: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Dropdown_setSelectedIndex");
            int index = getScalar<int>(prhs[2], "index");
        ...
        case /* hash32_fnv1a("hebi_charts_Dropdown_addOption") */ 3317650418: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Dropdown_addOption");
        ...
        case /* hash32_fnv1a("hebi_charts_Dropdown_wasChanged") */ 1278658951: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Dropdown_wasChanged");
        ...
        case /* hash32_fnv1a("hebi_charts_Dropdown_to_Control") */ 3115808627: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Dropdown_to_Control");
            void* _value = hebi_charts_Dropdown_to_Control(getPtr<void*>(prhs[1]));
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        ...
        case /* hash32_fnv1a("hebi_charts_Label_getText") */ 4217140664: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Label_getText");
        ...
        case /* hash32_fnv1a("hebi_charts_Label_setText") */ 2021464236: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Label_setText");
            MatlabString text(prhs[2]);
        ...
        case /* hash32_fnv1a("hebi_charts_Label_getValue") */ 4162135716: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Label_getValue");
        ...
        case /* hash32_fnv1a("hebi_charts_Label_setValue") */ 3763610232: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Label_setValue");
            double value = getScalar<double>(prhs[2], "value");
        ...
        case /* hash32_fnv1a("hebi_charts_Label_to_Control") */ 2160563954: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Label_to_Control");
            void* _value = hebi_charts_Label_to_Control(getPtr<void*>(prhs[1]));
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        ...
        case /* hash32_fnv1a("hebi_charts_Slider_setLimits") */ 358947112: {
            ...
            checkArgCount(nrhs, 3, "hebi_charts_Slider_setLimits");
            double min = getScalar<double>(prhs[2], "min");
            double max = getScalar<double>(prhs[3], "max");
        ...
        case /* hash32_fnv1a("hebi_charts_Slider_getMax") */ 443624594: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Slider_getMax");
        ...
        case /* hash32_fnv1a("hebi_charts_Slider_setMax") */ 1275740086: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Slider_setMax");
            double max = getScalar<double>(prhs[2], "max");
        ...
        case /* hash32_fnv1a("hebi_charts_Slider_getMin") */ 73237048: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Slider_getMin");
        ...
        case /* hash32_fnv1a("hebi_charts_Slider_setMin") */ 1576457300: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Slider_setMin");
            double min = getScalar<double>(prhs[2], "min");
        ...
        case /* hash32_fnv1a("hebi_charts_Slider_getValue") */ 2102644573: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Slider_getValue");
        ...
        case /* hash32_fnv1a("hebi_charts_Slider_setValue") */ 3045260073: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Slider_setValue");
            double value = getScalar<double>(prhs[2], "value");
        ...
        case /* hash32_fnv1a("hebi_charts_Slider_wasChanged") */ 2651043067: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Slider_wasChanged");
        ...
        case /* hash32_fnv1a("hebi_charts_Slider_to_Control") */ 474184687: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Slider_to_Control");
            void* _value = hebi_charts_Slider_to_Control(getPtr<void*>(prhs[1]));
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        ...
        case /* hash32_fnv1a("hebi_charts_Toggle_isSelected") */ 436557226: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Toggle_isSelected");
        ...
        case /* hash32_fnv1a("hebi_charts_Toggle_setSelected") */ 4261780332: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Toggle_setSelected");
            int selected = getScalar<int>(prhs[2], "selected");
        ...
        case /* hash32_fnv1a("hebi_charts_Toggle_wasChanged") */ 822243956: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Toggle_wasChanged");
        ...
        case /* hash32_fnv1a("hebi_charts_Toggle_to_Control") */ 660545256: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Toggle_to_Control");
            void* _value = hebi_charts_Toggle_to_Control(getPtr<void*>(prhs[1]));
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        ...
        case /* hash32_fnv1a("hebi_charts_ControlPanel_getTitle") */ 1927630722: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_ControlPanel_getTitle");
        ...
        case /* hash32_fnv1a("hebi_charts_ControlPanel_setTitle") */ 3220079598: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_ControlPanel_setTitle");
        ...
        case /* hash32_fnv1a("hebi_charts_ControlPanel_getWidth") */ 877042768: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_ControlPanel_getWidth");
        ...
        case /* hash32_fnv1a("hebi_charts_ControlPanel_setWidth") */ 2288203524: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_ControlPanel_setWidth");
            double width = getScalar<double>(prhs[2], "width");
        ...
        case /* hash32_fnv1a("hebi_charts_ControlPanel_addSection") */ 557108654: {
            static auto hebi_charts_ControlPanel_addSection = libLoader.getFunc<void(*)(ErrorInfo*, void*, const char*)>("hebi_charts_ControlPanel_addSection");
            checkArgCount(nrhs, 2, "hebi_charts_ControlPanel_addSection");
            ErrorInfo _error = {};
            MatlabString title(prhs[2]);
            hebi_charts_ControlPanel_addSection(&_error, getPtr<void*>(prhs[1]), title);
            checkError(_error);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_ControlPanel_addLabel") */ 4162567507: {
            static auto hebi_charts_ControlPanel_addLabel = libLoader.getFunc<void*(*)(ErrorInfo*, void*)>("hebi_charts_ControlPanel_addLabel");
            checkArgCount(nrhs, 1, "hebi_charts_ControlPanel_addLabel");
            ErrorInfo _error = {};
            void* _value = hebi_charts_ControlPanel_addLabel(&_error, getPtr<void*>(prhs[1]));
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_ControlPanel_addButton") */ 348400099: {
            static auto hebi_charts_ControlPanel_addButton = libLoader.getFunc<void*(*)(ErrorInfo*, void*)>("hebi_charts_ControlPanel_addButton");
            checkArgCount(nrhs, 1, "hebi_charts_ControlPanel_addButton");
            ErrorInfo _error = {};
            void* _value = hebi_charts_ControlPanel_addButton(&_error, getPtr<void*>(prhs[1]));
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_ControlPanel_addStartButton") */ 404094125: {
            static auto hebi_charts_ControlPanel_addStartButton = libLoader.getFunc<void*(*)(ErrorInfo*, void*)>("hebi_charts_ControlPanel_addStartButton");
            checkArgCount(nrhs, 1, "hebi_charts_ControlPanel_addStartButton");
            ErrorInfo _error = {};
            void* _value = hebi_charts_ControlPanel_addStartButton(&_error, getPtr<void*>(prhs[1]));
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_ControlPanel_addStopButton") */ 391801435: {
            static auto hebi_charts_ControlPanel_addStopButton = libLoader.getFunc<void*(*)(ErrorInfo*, void*)>("hebi_charts_ControlPanel_addStopButton");
            checkArgCount(nrhs, 1, "hebi_charts_ControlPanel_addStopButton");
            ErrorInfo _error = {};
            void* _value = hebi_charts_ControlPanel_addStopButton(&_error, getPtr<void*>(prhs[1]));
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_ControlPanel_addSlider") */ 1637676682: {
            static auto hebi_charts_ControlPanel_addSlider = libLoader.getFunc<void*(*)(ErrorInfo*, void*)>("hebi_charts_ControlPanel_addSlider");
            checkArgCount(nrhs, 1, "hebi_charts_ControlPanel_addSlider");
            ErrorInfo _error = {};
            void* _value = hebi_charts_ControlPanel_addSlider(&_error, getPtr<void*>(prhs[1]));
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_ControlPanel_addToggle") */ 1616396831: {
            static auto hebi_charts_ControlPanel_addToggle = libLoader.getFunc<void*(*)(ErrorInfo*, void*)>("hebi_charts_ControlPanel_addToggle");
            checkArgCount(nrhs, 1, "hebi_charts_ControlPanel_addToggle");
            ErrorInfo _error = {};
            void* _value = hebi_charts_ControlPanel_addToggle(&_error, getPtr<void*>(prhs[1]));
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_ControlPanel_addDropdown") */ 2499277402: {
            static auto hebi_charts_ControlPanel_addDropdown = libLoader.getFunc<void*(*)(ErrorInfo*, void*)>("hebi_charts_ControlPanel_addDropdown");
            checkArgCount(nrhs, 1, "hebi_charts_ControlPanel_addDropdown");
            ErrorInfo _error = {};
            void* _value = hebi_charts_ControlPanel_addDropdown(&_error, getPtr<void*>(prhs[1]));
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_ControlPanel_release") */ 4196057477: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_ControlPanel_release");
        ...
        case /* hash32_fnv1a("hebi_charts_Cursor_isEditable") */ 3545168017: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Cursor_isEditable");
        ...
        case /* hash32_fnv1a("hebi_charts_Cursor_setEditable") */ 1606003675: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Cursor_setEditable");
            int editable = getScalar<int>(prhs[2], "editable");
        ...
        case /* hash32_fnv1a("hebi_charts_Cursor_getLabel") */ 2513275231: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Cursor_getLabel");
        ...
        case /* hash32_fnv1a("hebi_charts_Cursor_setLabel") */ 2912272059: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Cursor_setLabel");
        ...
        case /* hash32_fnv1a("hebi_charts_Cursor_getValue") */ 1660839612: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Cursor_getValue");
        ...
        case /* hash32_fnv1a("hebi_charts_Cursor_setValue") */ 3942528016: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Cursor_setValue");
            double value = getScalar<double>(prhs[2], "value");
        ...
        case /* hash32_fnv1a("hebi_charts_Cursor_isVisible") */ 2958376865: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Cursor_isVisible");
        ...
        case /* hash32_fnv1a("hebi_charts_Cursor_setVisible") */ 3195068407: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Cursor_setVisible");
            int visible = getScalar<int>(prhs[2], "visible");
        ...
        case /* hash32_fnv1a("hebi_charts_Cursor_release") */ 1029829710: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Cursor_release");
        ...
        case /* hash32_fnv1a("hebi_charts_FxmlView_isAutoReload") */ 3201681499: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_FxmlView_isAutoReload");
        ...
        case /* hash32_fnv1a("hebi_charts_FxmlView_setAutoReload") */ 3760019613: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_FxmlView_setAutoReload");
            int enableAutoReload = getScalar<int>(prhs[2], "enableAutoReload");
        ...
        case /* hash32_fnv1a("hebi_charts_FxmlView_getSource") */ 109946018: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_FxmlView_getSource");
        ...
        case /* hash32_fnv1a("hebi_charts_FxmlView_setSource") */ 3778040214: {
            static auto hebi_charts_FxmlView_setSource = libLoader.getFunc<void(*)(ErrorInfo*, void*, const char*)>("hebi_charts_FxmlView_setSource");
            checkArgCount(nrhs, 2, "hebi_charts_FxmlView_setSource");
            ErrorInfo _error = {};
            MatlabString pathOrUrl(prhs[2]);
            hebi_charts_FxmlView_setSource(&_error, getPtr<void*>(prhs[1]), pathOrUrl);
            checkError(_error);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_FxmlView_addLineChart") */ 576685690: {
            static auto hebi_charts_FxmlView_addLineChart = libLoader.getFunc<void*(*)(ErrorInfo*, void*, const char*)>("hebi_charts_FxmlView_addLineChart");
            checkArgCount(nrhs, 2, "hebi_charts_FxmlView_addLineChart");
            ErrorInfo _error = {};
            MatlabString fxId(prhs[2]);
            void* _value = hebi_charts_FxmlView_addLineChart(&_error, getPtr<void*>(prhs[1]), fxId);
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_FxmlView_addScope") */ 288888368: {
            static auto hebi_charts_FxmlView_addScope = libLoader.getFunc<void*(*)(ErrorInfo*, void*, const char*)>("hebi_charts_FxmlView_addScope");
            checkArgCount(nrhs, 2, "hebi_charts_FxmlView_addScope");
            ErrorInfo _error = {};
            MatlabString fxId(prhs[2]);
            void* _value = hebi_charts_FxmlView_addScope(&_error, getPtr<void*>(prhs[1]), fxId);
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_FxmlView_addLatencyChart") */ 2361709490: {
            static auto hebi_charts_FxmlView_addLatencyChart = libLoader.getFunc<void*(*)(ErrorInfo*, void*, const char*)>("hebi_charts_FxmlView_addLatencyChart");
            checkArgCount(nrhs, 2, "hebi_charts_FxmlView_addLatencyChart");
            ErrorInfo _error = {};
            MatlabString fxId(prhs[2]);
            void* _value = hebi_charts_FxmlView_addLatencyChart(&_error, getPtr<void*>(prhs[1]), fxId);
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_FxmlView_addScene3d") */ 3960459659: {
            static auto hebi_charts_FxmlView_addScene3d = libLoader.getFunc<void*(*)(ErrorInfo*, void*, const char*)>("hebi_charts_FxmlView_addScene3d");
            checkArgCount(nrhs, 2, "hebi_charts_FxmlView_addScene3d");
            ErrorInfo _error = {};
            MatlabString fxId(prhs[2]);
            void* _value = hebi_charts_FxmlView_addScene3d(&_error, getPtr<void*>(prhs[1]), fxId);
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_FxmlView_addStreamView") */ 3468796213: {
            static auto hebi_charts_FxmlView_addStreamView = libLoader.getFunc<void*(*)(ErrorInfo*, void*, const char*, const char*)>("hebi_charts_FxmlView_addStreamView");
            checkArgCount(nrhs, 3, "hebi_charts_FxmlView_addStreamView");
            ErrorInfo _error = {};
            MatlabString file(prhs[2]);
            MatlabString fxId(prhs[3]);
            void* _value = hebi_charts_FxmlView_addStreamView(&_error, getPtr<void*>(prhs[1]), file, fxId);
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_FxmlView_addFxmlView") */ 3279362014: {
            static auto hebi_charts_FxmlView_addFxmlView = libLoader.getFunc<void*(*)(ErrorInfo*, void*, const char*)>("hebi_charts_FxmlView_addFxmlView");
            checkArgCount(nrhs, 2, "hebi_charts_FxmlView_addFxmlView");
            ErrorInfo _error = {};
            MatlabString fxId(prhs[2]);
            void* _value = hebi_charts_FxmlView_addFxmlView(&_error, getPtr<void*>(prhs[1]), fxId);
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_FxmlView_release") */ 1439298174: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_FxmlView_release");
        ...
        case /* hash32_fnv1a("hebi_charts_GridWindow_create") */ 481105715: {
            static auto hebi_charts_GridWindow_create = libLoader.getFunc<void*(*)(ErrorInfo*, int, int)>("hebi_charts_GridWindow_create");
            checkArgCount(nrhs, 2, "hebi_charts_GridWindow_create");
            ErrorInfo _error = {};
            int rows = getScalar<int>(prhs[1], "rows");
            int cols = getScalar<int>(prhs[2], "cols");
            void* _value = hebi_charts_GridWindow_create(&_error, rows, cols);
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_GridWindow_isFullScreen") */ 1721034094: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_GridWindow_isFullScreen");
        ...
        case /* hash32_fnv1a("hebi_charts_GridWindow_setFullScreen") */ 1125370908: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_GridWindow_setFullScreen");
            int fullScreen = getScalar<int>(prhs[2], "fullScreen");
        ...
        case /* hash32_fnv1a("hebi_charts_GridWindow_getHeight") */ 1740977914: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_GridWindow_getHeight");
        ...
        case /* hash32_fnv1a("hebi_charts_GridWindow_setHeight") */ 2254829894: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_GridWindow_setHeight");
            int height = getScalar<int>(prhs[2], "height");
        ...
        case /* hash32_fnv1a("hebi_charts_GridWindow_getKeepOpen") */ 2380182568: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_GridWindow_getKeepOpen");
        ...
        case /* hash32_fnv1a("hebi_charts_GridWindow_setKeepOpen") */ 140990388: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_GridWindow_setKeepOpen");
            int keepOpen = getScalar<int>(prhs[2], "keepOpen");
        ...
        case /* hash32_fnv1a("hebi_charts_GridWindow_setLocation") */ 1129240730: {
            ...
            checkArgCount(nrhs, 3, "hebi_charts_GridWindow_setLocation");
            int xOffset = getScalar<int>(prhs[2], "xOffset");
            int yOffset = getScalar<int>(prhs[3], "yOffset");
        ...
        case /* hash32_fnv1a("hebi_charts_GridWindow_setSize") */ 1704531216: {
            ...
            checkArgCount(nrhs, 3, "hebi_charts_GridWindow_setSize");
            int width = getScalar<int>(prhs[2], "width");
            int height = getScalar<int>(prhs[3], "height");
        ...
        case /* hash32_fnv1a("hebi_charts_GridWindow_getTitle") */ 1153585057: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_GridWindow_getTitle");
        ...
        case /* hash32_fnv1a("hebi_charts_GridWindow_setTitle") */ 2959599629: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_GridWindow_setTitle");
            MatlabString title(prhs[2]);
        ...
        case /* hash32_fnv1a("hebi_charts_GridWindow_getWidth") */ 1311524919: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_GridWindow_getWidth");
        ...
        case /* hash32_fnv1a("hebi_charts_GridWindow_setWidth") */ 782365675: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_GridWindow_setWidth");
            int width = getScalar<int>(prhs[2], "width");
        ...
        case /* hash32_fnv1a("hebi_charts_GridWindow_getX") */ 2878897391: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_GridWindow_getX");
        ...
        case /* hash32_fnv1a("hebi_charts_GridWindow_setX") */ 4248629307: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_GridWindow_setX");
            int x = getScalar<int>(prhs[2], "x");
        ...
        case /* hash32_fnv1a("hebi_charts_GridWindow_getY") */ 2862119772: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_GridWindow_getY");
        ...
        case /* hash32_fnv1a("hebi_charts_GridWindow_setY") */ 4231851688: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_GridWindow_setY");
            int y = getScalar<int>(prhs[2], "y");
            hebi_charts_GridWindow_setY(getPtr<void*>(prhs[1]), y);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_GridWindow_addLineChart") */ 3022891796: {
            static auto hebi_charts_GridWindow_addLineChart = libLoader.getFunc<void*(*)(ErrorInfo*, void*, int, int, int, int)>("hebi_charts_GridWindow_addLineChart");
            checkArgCount(nrhs, 5, "hebi_charts_GridWindow_addLineChart");
            ErrorInfo _error = {};
            int row = getScalar<int>(prhs[2], "row");
            int col = getScalar<int>(prhs[3], "col");
            int rowSpan = getScalar<int>(prhs[4], "rowSpan");
            int colSpan = getScalar<int>(prhs[5], "colSpan");
            void* _value = hebi_charts_GridWindow_addLineChart(&_error, getPtr<void*>(prhs[1]), row, col, rowSpan, colSpan);
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_GridWindow_addScope") */ 1120848738: {
            static auto hebi_charts_GridWindow_addScope = libLoader.getFunc<void*(*)(ErrorInfo*, void*, int, int, int, int)>("hebi_charts_GridWindow_addScope");
            checkArgCount(nrhs, 5, "hebi_charts_GridWindow_addScope");
            ErrorInfo _error = {};
            int row = getScalar<int>(prhs[2], "row");
            int col = getScalar<int>(prhs[3], "col");
            int rowSpan = getScalar<int>(prhs[4], "rowSpan");
            int colSpan = getScalar<int>(prhs[5], "colSpan");
            void* _value = hebi_charts_GridWindow_addScope(&_error, getPtr<void*>(prhs[1]), row, col, rowSpan, colSpan);
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_GridWindow_addLatencyChart") */ 2808728484: {
            static auto hebi_charts_GridWindow_addLatencyChart = libLoader.getFunc<void*(*)(ErrorInfo*, void*, int, int, int, int)>("hebi_charts_GridWindow_addLatencyChart");
            checkArgCount(nrhs, 5, "hebi_charts_GridWindow_addLatencyChart");
            ErrorInfo _error = {};
            int row = getScalar<int>(prhs[2], "row");
            int col = getScalar<int>(prhs[3], "col");
            int rowSpan = getScalar<int>(prhs[4], "rowSpan");
            int colSpan = getScalar<int>(prhs[5], "colSpan");
            void* _value = hebi_charts_GridWindow_addLatencyChart(&_error, getPtr<void*>(prhs[1]), row, col, rowSpan, colSpan);
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_GridWindow_addScene3d") */ 994315921: {
            static auto hebi_charts_GridWindow_addScene3d = libLoader.getFunc<void*(*)(ErrorInfo*, void*, int, int, int, int)>("hebi_charts_GridWindow_addScene3d");
            checkArgCount(nrhs, 5, "hebi_charts_GridWindow_addScene3d");
            ErrorInfo _error = {};
            int row = getScalar<int>(prhs[2], "row");
            int col = getScalar<int>(prhs[3], "col");
            int rowSpan = getScalar<int>(prhs[4], "rowSpan");
            int colSpan = getScalar<int>(prhs[5], "colSpan");
            void* _value = hebi_charts_GridWindow_addScene3d(&_error, getPtr<void*>(prhs[1]), row, col, rowSpan, colSpan);
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_GridWindow_addStreamView") */ 2740676711: {
            static auto hebi_charts_GridWindow_addStreamView = libLoader.getFunc<void*(*)(ErrorInfo*, void*, const char*, int, int, int, int)>("hebi_charts_GridWindow_addStreamView");
            checkArgCount(nrhs, 6, "hebi_charts_GridWindow_addStreamView");
            ErrorInfo _error = {};
            MatlabString file(prhs[2]);
            int row = getScalar<int>(prhs[3], "row");
            int col = getScalar<int>(prhs[4], "col");
            int rowSpan = getScalar<int>(prhs[5], "rowSpan");
            int colSpan = getScalar<int>(prhs[6], "colSpan");
            void* _value = hebi_charts_GridWindow_addStreamView(&_error, getPtr<void*>(prhs[1]), file, row, col, rowSpan, colSpan);
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_GridWindow_addFxmlView") */ 494287860: {
            static auto hebi_charts_GridWindow_addFxmlView = libLoader.getFunc<void*(*)(ErrorInfo*, void*, int, int, int, int)>("hebi_charts_GridWindow_addFxmlView");
            checkArgCount(nrhs, 5, "hebi_charts_GridWindow_addFxmlView");
            ErrorInfo _error = {};
            int row = getScalar<int>(prhs[2], "row");
            int col = getScalar<int>(prhs[3], "col");
            int rowSpan = getScalar<int>(prhs[4], "rowSpan");
            int colSpan = getScalar<int>(prhs[5], "colSpan");
            void* _value = hebi_charts_GridWindow_addFxmlView(&_error, getPtr<void*>(prhs[1]), row, col, rowSpan, colSpan);
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_GridWindow_addStylesheet") */ 2208938158: {
            static auto hebi_charts_GridWindow_addStylesheet = libLoader.getFunc<void(*)(ErrorInfo*, void*, const char*, int)>("hebi_charts_GridWindow_addStylesheet");
            checkArgCount(nrhs, 3, "hebi_charts_GridWindow_addStylesheet");
            ErrorInfo _error = {};
            MatlabString pathOrUrl(prhs[2]);
            int autoReload = getScalar<int>(prhs[3], "autoReload");
            hebi_charts_GridWindow_addStylesheet(&_error, getPtr<void*>(prhs[1]), pathOrUrl, autoReload);
            checkError(_error);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_GridWindow_show") */ 571378966: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_GridWindow_show");
            hebi_charts_GridWindow_show(getPtr<void*>(prhs[1]));
            break;
        }
        case /* hash32_fnv1a("hebi_charts_GridWindow_showOffScreen") */ 1460317711: {
            static auto hebi_charts_GridWindow_showOffScreen = libLoader.getFunc<void(*)(ErrorInfo*, void*)>("hebi_charts_GridWindow_showOffScreen");
            checkArgCount(nrhs, 1, "hebi_charts_GridWindow_showOffScreen");
            ErrorInfo _error = {};
            hebi_charts_GridWindow_showOffScreen(&_error, getPtr<void*>(prhs[1]));
            checkError(_error);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_GridWindow_hide") */ 3322641491: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_GridWindow_hide");
        ...
        case /* hash32_fnv1a("hebi_charts_GridWindow_isShowing") */ 220171030: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_GridWindow_isShowing");
        ...
        case /* hash32_fnv1a("hebi_charts_GridWindow_waitUntilClosed") */ 1606158494: {
            static auto hebi_charts_GridWindow_waitUntilClosed = libLoader.getFunc<void(*)(ErrorInfo*, void*)>("hebi_charts_GridWindow_waitUntilClosed");
            checkArgCount(nrhs, 1, "hebi_charts_GridWindow_waitUntilClosed");
            ErrorInfo _error = {};
            hebi_charts_GridWindow_waitUntilClosed(&_error, getPtr<void*>(prhs[1]));
            checkError(_error);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_GridWindow_getControlPanel") */ 2796496580: {
            static auto hebi_charts_GridWindow_getControlPanel = libLoader.getFunc<void*(*)(ErrorInfo*, void*)>("hebi_charts_GridWindow_getControlPanel");
            checkArgCount(nrhs, 1, "hebi_charts_GridWindow_getControlPanel");
            ErrorInfo _error = {};
            void* _value = hebi_charts_GridWindow_getControlPanel(&_error, getPtr<void*>(prhs[1]));
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_GridWindow_createImageStream") */ 2319112984: {
            static auto hebi_charts_GridWindow_createImageStream = libLoader.getFunc<void*(*)(ErrorInfo*, void*)>("hebi_charts_GridWindow_createImageStream");
            checkArgCount(nrhs, 1, "hebi_charts_GridWindow_createImageStream");
            ErrorInfo _error = {};
            void* _value = hebi_charts_GridWindow_createImageStream(&_error, getPtr<void*>(prhs[1]));
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_GridWindow_dispatchMouseEvent") */ 493138856: {
            ...
            checkArgCount(nrhs, 7, "hebi_charts_GridWindow_dispatchMouseEvent");
            int action = getScalar<int>(prhs[2], "action");
            int button = getScalar<int>(prhs[3], "button");
            int downMask = getScalar<int>(prhs[4], "downMask");
            int modifiers = getScalar<int>(prhs[5], "modifiers");
            double x = getScalar<double>(prhs[6], "x");
            double y = getScalar<double>(prhs[7], "y");
        ...
        case /* hash32_fnv1a("hebi_charts_GridWindow_dispatchScrollEvent") */ 3322726912: {
            ...
            checkArgCount(nrhs, 6, "hebi_charts_GridWindow_dispatchScrollEvent");
            double x = getScalar<double>(prhs[2], "x");
            double y = getScalar<double>(prhs[3], "y");
            double delta_x = getScalar<double>(prhs[4], "delta_x");
            double delta_y = getScalar<double>(prhs[5], "delta_y");
            int modifiers = getScalar<int>(prhs[6], "modifiers");
        ...
        case /* hash32_fnv1a("hebi_charts_GridWindow_release") */ 288831216: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_GridWindow_release");
        ...
        case /* hash32_fnv1a("hebi_charts_HdrHistogramRecorder_create") */ 513492597: {
            static auto hebi_charts_HdrHistogramRecorder_create = libLoader.getFunc<void*(*)(ErrorInfo*)>("hebi_charts_HdrHistogramRecorder_create");
            checkArgCount(nrhs, 0, "hebi_charts_HdrHistogramRecorder_create");
            ErrorInfo _error = {};
            void* _value = hebi_charts_HdrHistogramRecorder_create(&_error);
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_HdrHistogramRecorder_getFrequency") */ 3235251207: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_HdrHistogramRecorder_getFrequency");
        ...
        case /* hash32_fnv1a("hebi_charts_HdrHistogramRecorder_setFrequency") */ 126668259: {
            static auto hebi_charts_HdrHistogramRecorder_setFrequency = libLoader.getFunc<void(*)(ErrorInfo*, void*, double)>("hebi_charts_HdrHistogramRecorder_setFrequency");
            checkArgCount(nrhs, 2, "hebi_charts_HdrHistogramRecorder_setFrequency");
            ErrorInfo _error = {};
            double frequency = getScalar<double>(prhs[2], "frequency");
            hebi_charts_HdrHistogramRecorder_setFrequency(&_error, getPtr<void*>(prhs[1]), frequency);
            checkError(_error);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_HdrHistogramRecorder_getMax") */ 1760728599: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_HdrHistogramRecorder_getMax");
        ...
        case /* hash32_fnv1a("hebi_charts_HdrHistogramRecorder_setMax") */ 1697553923: {
            static auto hebi_charts_HdrHistogramRecorder_setMax = libLoader.getFunc<void(*)(ErrorInfo*, void*, double)>("hebi_charts_HdrHistogramRecorder_setMax");
            checkArgCount(nrhs, 2, "hebi_charts_HdrHistogramRecorder_setMax");
            ErrorInfo _error = {};
            double value = getScalar<double>(prhs[2], "value");
            hebi_charts_HdrHistogramRecorder_setMax(&_error, getPtr<void*>(prhs[1]), value);
            checkError(_error);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_HdrHistogramRecorder_getMin") */ 1460011385: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_HdrHistogramRecorder_getMin");
        ...
        case /* hash32_fnv1a("hebi_charts_HdrHistogramRecorder_setMin") */ 1531057661: {
            static auto hebi_charts_HdrHistogramRecorder_setMin = libLoader.getFunc<void(*)(ErrorInfo*, void*, double)>("hebi_charts_HdrHistogramRecorder_setMin");
            checkArgCount(nrhs, 2, "hebi_charts_HdrHistogramRecorder_setMin");
            ErrorInfo _error = {};
            double value = getScalar<double>(prhs[2], "value");
            hebi_charts_HdrHistogramRecorder_setMin(&_error, getPtr<void*>(prhs[1]), value);
            checkError(_error);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_HdrHistogramRecorder_getSignificantDigits") */ 2205589608: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_HdrHistogramRecorder_getSignificantDigits");
        ...
        case /* hash32_fnv1a("hebi_charts_HdrHistogramRecorder_setSignificantDigits") */ 4180067068: {
            static auto hebi_charts_HdrHistogramRecorder_setSignificantDigits = libLoader.getFunc<void(*)(ErrorInfo*, void*, int)>("hebi_charts_HdrHistogramRecorder_setSignificantDigits");
            checkArgCount(nrhs, 2, "hebi_charts_HdrHistogramRecorder_setSignificantDigits");
            ErrorInfo _error = {};
            int significantDigits = getScalar<int>(prhs[2], "significantDigits");
            hebi_charts_HdrHistogramRecorder_setSignificantDigits(&_error, getPtr<void*>(prhs[1]), significantDigits);
            checkError(_error);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_HdrHistogramRecorder_addTrace") */ 3486503389: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_HdrHistogramRecorder_addTrace");
        ...
        case /* hash32_fnv1a("hebi_charts_HdrHistogramRecorder_startRecording") */ 3232561954: {
            static auto hebi_charts_HdrHistogramRecorder_startRecording = libLoader.getFunc<const char*(*)(ErrorInfo*, void*, const char*)>("hebi_charts_HdrHistogramRecorder_startRecording");
            checkArgCount(nrhs, 2, "hebi_charts_HdrHistogramRecorder_startRecording");
            ErrorInfo _error = {};
            MatlabString logFile(prhs[2]);
            const char* _value = hebi_charts_HdrHistogramRecorder_startRecording(&_error, getPtr<void*>(prhs[1]), logFile);
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexString(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_HdrHistogramRecorder_stopRecording") */ 4037906548: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_HdrHistogramRecorder_stopRecording");
        ...
        case /* hash32_fnv1a("hebi_charts_HdrHistogramRecorder_release") */ 1289531094: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_HdrHistogramRecorder_release");
        ...
        case /* hash32_fnv1a("hebi_charts_HdrHistogramTrace_getMax") */ 3723481894: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_HdrHistogramTrace_getMax");
        ...
        case /* hash32_fnv1a("hebi_charts_HdrHistogramTrace_getMean") */ 409566793: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_HdrHistogramTrace_getMean");
        ...
        case /* hash32_fnv1a("hebi_charts_HdrHistogramTrace_getMin") */ 3487315300: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_HdrHistogramTrace_getMin");
        ...
        case /* hash32_fnv1a("hebi_charts_HdrHistogramTrace_getStdDeviation") */ 859032600: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_HdrHistogramTrace_getStdDeviation");
        ...
        case /* hash32_fnv1a("hebi_charts_HdrHistogramTrace_getTag") */ 2691889208: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_HdrHistogramTrace_getTag");
        ...
        case /* hash32_fnv1a("hebi_charts_HdrHistogramTrace_getTotalCount") */ 1620338425: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_HdrHistogramTrace_getTotalCount");
            size_t _value = hebi_charts_HdrHistogramTrace_getTotalCount(getPtr<void*>(prhs[1]));
            if (nlhs > 0) plhs[0] = createMexSize(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_HdrHistogramTrace_createLocal") */ 1127795715: {
            ...
            checkArgCount(nrhs, 3, "hebi_charts_HdrHistogramTrace_createLocal");
            int numberOfSignificantDigits = getScalar<int>(prhs[1], "numberOfSignificantDigits");
            double minSeconds = getScalar<double>(prhs[2], "minSeconds");
            double maxSeconds = getScalar<double>(prhs[3], "maxSeconds");
        ...
        case /* hash32_fnv1a("hebi_charts_HdrHistogramTrace_getValueAtPercentile") */ 2933860591: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_HdrHistogramTrace_getValueAtPercentile");
            double percentile = getScalar<double>(prhs[2], "percentile");
        ...
        case /* hash32_fnv1a("hebi_charts_HdrHistogramTrace_tic") */ 3683884192: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_HdrHistogramTrace_tic");
        ...
        case /* hash32_fnv1a("hebi_charts_HdrHistogramTrace_toc") */ 60904226: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_HdrHistogramTrace_toc");
        ...
        case /* hash32_fnv1a("hebi_charts_HdrHistogramTrace_ticToc") */ 4060989918: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_HdrHistogramTrace_ticToc");
        ...
        case /* hash32_fnv1a("hebi_charts_HdrHistogramTrace_recordValue") */ 961953446: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_HdrHistogramTrace_recordValue");
            double value = getScalar<double>(prhs[2], "value");
        ...
        case /* hash32_fnv1a("hebi_charts_HdrHistogramTrace_recordValueWithCount") */ 1955216297: {
            ...
            checkArgCount(nrhs, 3, "hebi_charts_HdrHistogramTrace_recordValueWithCount");
            double value = getScalar<double>(prhs[2], "value");
            size_t count = getScalar<size_t>(prhs[3], "count");
        ...
        case /* hash32_fnv1a("hebi_charts_HdrHistogramTrace_recordValueWithExpectedInterval") */ 595474035: {
            ...
            checkArgCount(nrhs, 3, "hebi_charts_HdrHistogramTrace_recordValueWithExpectedInterval");
            double value = getScalar<double>(prhs[2], "value");
            double expectedIntervalBetweenSamples = getScalar<double>(prhs[3], "expectedIntervalBetweenSamples");
        ...
        case /* hash32_fnv1a("hebi_charts_HdrHistogramTrace_reset") */ 778242123: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_HdrHistogramTrace_reset");
        ...
        case /* hash32_fnv1a("hebi_charts_HdrHistogramTrace_toHgrmString") */ 1378221180: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_HdrHistogramTrace_toHgrmString");
            double outputUnitsPerSecond = getScalar<double>(prhs[2], "outputUnitsPerSecond");
        ...
        case /* hash32_fnv1a("hebi_charts_HdrHistogramTrace_saveAsHgrm") */ 1639013757: {
            static auto hebi_charts_HdrHistogramTrace_saveAsHgrm = libLoader.getFunc<const char*(*)(ErrorInfo*, void*, const char*, double)>("hebi_charts_HdrHistogramTrace_saveAsHgrm");
            checkArgCount(nrhs, 3, "hebi_charts_HdrHistogramTrace_saveAsHgrm");
            ErrorInfo _error = {};
            MatlabString fileName(prhs[2]);
            double outputUnitsPerSecond = getScalar<double>(prhs[3], "outputUnitsPerSecond");
            const char* _value = hebi_charts_HdrHistogramTrace_saveAsHgrm(&_error, getPtr<void*>(prhs[1]), fileName, outputUnitsPerSecond);
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexString(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_HdrHistogramTrace_release") */ 2275312785: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_HdrHistogramTrace_release");
        ...
        case /* hash32_fnv1a("hebi_charts_ImageStream_getBuffer") */ 2670059014: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_ImageStream_getBuffer");
        ...
        case /* hash32_fnv1a("hebi_charts_ImageStream_getBufferSize") */ 4286272295: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_ImageStream_getBufferSize");
        ...
        case /* hash32_fnv1a("hebi_charts_ImageStream_getChannels") */ 2905113076: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_ImageStream_getChannels");
        ...
        case /* hash32_fnv1a("hebi_charts_ImageStream_getHeight") */ 449632559: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_ImageStream_getHeight");
        ...
        case /* hash32_fnv1a("hebi_charts_ImageStream_getPixelFormat") */ 1231628591: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_ImageStream_getPixelFormat");
        ...
        case /* hash32_fnv1a("hebi_charts_ImageStream_setPixelFormat") */ 3498113259: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_ImageStream_setPixelFormat");
            int pixelFormat = getScalar<int>(prhs[2], "pixelFormat");
        ...
        case /* hash32_fnv1a("hebi_charts_ImageStream_getRateLimit") */ 3218724877: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_ImageStream_getRateLimit");
        ...
        case /* hash32_fnv1a("hebi_charts_ImageStream_setRateLimit") */ 2797088897: {
            static auto hebi_charts_ImageStream_setRateLimit = libLoader.getFunc<void(*)(ErrorInfo*, void*, double)>("hebi_charts_ImageStream_setRateLimit");
            checkArgCount(nrhs, 2, "hebi_charts_ImageStream_setRateLimit");
            ErrorInfo _error = {};
            double maxFramesPerSecond = getScalar<double>(prhs[2], "maxFramesPerSecond");
            hebi_charts_ImageStream_setRateLimit(&_error, getPtr<void*>(prhs[1]), maxFramesPerSecond);
            checkError(_error);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_ImageStream_getRecorderThreads") */ 4046330431: {
            static auto hebi_charts_ImageStream_getRecorderThreads = libLoader.getFunc<size_t(*)(ErrorInfo*, void*)>("hebi_charts_ImageStream_getRecorderThreads");
            checkArgCount(nrhs, 1, "hebi_charts_ImageStream_getRecorderThreads");
            ErrorInfo _error = {};
            size_t _value = hebi_charts_ImageStream_getRecorderThreads(&_error, getPtr<void*>(prhs[1]));
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexSize(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_ImageStream_setRecorderThreads") */ 1737193747: {
            static auto hebi_charts_ImageStream_setRecorderThreads = libLoader.getFunc<void(*)(ErrorInfo*, void*, size_t)>("hebi_charts_ImageStream_setRecorderThreads");
            checkArgCount(nrhs, 2, "hebi_charts_ImageStream_setRecorderThreads");
            ErrorInfo _error = {};
            size_t numThreads = getScalar<size_t>(prhs[2], "numThreads");
            hebi_charts_ImageStream_setRecorderThreads(&_error, getPtr<void*>(prhs[1]), numThreads);
            checkError(_error);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_ImageStream_isRecording") */ 1217846067: {
            static auto hebi_charts_ImageStream_isRecording = libLoader.getFunc<int(*)(ErrorInfo*, void*)>("hebi_charts_ImageStream_isRecording");
            checkArgCount(nrhs, 1, "hebi_charts_ImageStream_isRecording");
            ErrorInfo _error = {};
            int _value = hebi_charts_ImageStream_isRecording(&_error, getPtr<void*>(prhs[1]));
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexScalar(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_ImageStream_getRenderScale") */ 1687894102: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_ImageStream_getRenderScale");
        ...
        case /* hash32_fnv1a("hebi_charts_ImageStream_getRenderScaleX") */ 1851342346: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_ImageStream_getRenderScaleX");
        ...
        case /* hash32_fnv1a("hebi_charts_ImageStream_getRenderScaleY") */ 1868119965: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_ImageStream_getRenderScaleY");
        ...
        case /* hash32_fnv1a("hebi_charts_ImageStream_getSequence") */ 3666491377: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_ImageStream_getSequence");
        ...
        case /* hash32_fnv1a("hebi_charts_ImageStream_getStride") */ 2883829305: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_ImageStream_getStride");
        ...
        case /* hash32_fnv1a("hebi_charts_ImageStream_getTimestamp") */ 4221629260: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_ImageStream_getTimestamp");
        ...
        case /* hash32_fnv1a("hebi_charts_ImageStream_getWidth") */ 2250410136: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_ImageStream_getWidth");
        ...
        case /* hash32_fnv1a("hebi_charts_ImageStream_setResolution") */ 1812315458: {
            static auto hebi_charts_ImageStream_setResolution = libLoader.getFunc<void(*)(ErrorInfo*, void*, int, int)>("hebi_charts_ImageStream_setResolution");
            checkArgCount(nrhs, 3, "hebi_charts_ImageStream_setResolution");
            ErrorInfo _error = {};
            int width = getScalar<int>(prhs[2], "width");
            int height = getScalar<int>(prhs[3], "height");
            hebi_charts_ImageStream_setResolution(&_error, getPtr<void*>(prhs[1]), width, height);
            checkError(_error);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_ImageStream_waitForNext") */ 996531661: {
            static auto hebi_charts_ImageStream_waitForNext = libLoader.getFunc<int(*)(ErrorInfo*, void*, size_t)>("hebi_charts_ImageStream_waitForNext");
            checkArgCount(nrhs, 2, "hebi_charts_ImageStream_waitForNext");
            ErrorInfo _error = {};
            size_t maxTimeoutMillis = getScalar<size_t>(prhs[2], "maxTimeoutMillis");
            int _value = hebi_charts_ImageStream_waitForNext(&_error, getPtr<void*>(prhs[1]), maxTimeoutMillis);
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexScalar(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_ImageStream_tryGetNext") */ 1851634994: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_ImageStream_tryGetNext");
        ...
        case /* hash32_fnv1a("hebi_charts_ImageStream_startRecording") */ 2959520791: {
            static auto hebi_charts_ImageStream_startRecording = libLoader.getFunc<void(*)(ErrorInfo*, void*, const char*, int)>("hebi_charts_ImageStream_startRecording");
            checkArgCount(nrhs, 3, "hebi_charts_ImageStream_startRecording");
            ErrorInfo _error = {};
            MatlabString baseName(prhs[2]);
            int overwrite = getScalar<int>(prhs[3], "overwrite");
            hebi_charts_ImageStream_startRecording(&_error, getPtr<void*>(prhs[1]), baseName, overwrite);
            checkError(_error);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_ImageStream_stopRecording") */ 451110275: {
            static auto hebi_charts_ImageStream_stopRecording = libLoader.getFunc<void*(*)(ErrorInfo*, void*)>("hebi_charts_ImageStream_stopRecording");
            checkArgCount(nrhs, 1, "hebi_charts_ImageStream_stopRecording");
            ErrorInfo _error = {};
            void* _value = hebi_charts_ImageStream_stopRecording(&_error, getPtr<void*>(prhs[1]));
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_ImageStream_saveToFile") */ 3426388426: {
            static auto hebi_charts_ImageStream_saveToFile = libLoader.getFunc<void(*)(ErrorInfo*, void*, const char*)>("hebi_charts_ImageStream_saveToFile");
            checkArgCount(nrhs, 2, "hebi_charts_ImageStream_saveToFile");
            ErrorInfo _error = {};
            MatlabString fileName(prhs[2]);
            hebi_charts_ImageStream_saveToFile(&_error, getPtr<void*>(prhs[1]), fileName);
            checkError(_error);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_ImageStream_release") */ 2907127757: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_ImageStream_release");
        ...
        case /* hash32_fnv1a("hebi_charts_LoopTimer_create") */ 3391059830: {
            static auto hebi_charts_LoopTimer_create = libLoader.getFunc<void*(*)(ErrorInfo*)>("hebi_charts_LoopTimer_create");
            checkArgCount(nrhs, 0, "hebi_charts_LoopTimer_create");
            ErrorInfo _error = {};
            void* _value = hebi_charts_LoopTimer_create(&_error);
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_LoopTimer_getElapsedTime") */ 1842144231: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_LoopTimer_getElapsedTime");
        ...
        case /* hash32_fnv1a("hebi_charts_LoopTimer_getFrequency") */ 740806432: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_LoopTimer_getFrequency");
        ...
        case /* hash32_fnv1a("hebi_charts_LoopTimer_setFrequency") */ 2423821956: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_LoopTimer_setFrequency");
            double frequency = getScalar<double>(prhs[2], "frequency");
        ...
        case /* hash32_fnv1a("hebi_charts_LoopTimer_getPeriod") */ 2990288999: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_LoopTimer_getPeriod");
        ...
        case /* hash32_fnv1a("hebi_charts_LoopTimer_setPeriod") */ 1951692307: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_LoopTimer_setPeriod");
            double seconds = getScalar<double>(prhs[2], "seconds");
        ...
        case /* hash32_fnv1a("hebi_charts_LoopTimer_restart") */ 1382219461: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_LoopTimer_restart");
        ...
        case /* hash32_fnv1a("hebi_charts_LoopTimer_tic") */ 4074232662: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_LoopTimer_tic");
        ...
        case /* hash32_fnv1a("hebi_charts_LoopTimer_toc") */ 4208850972: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_LoopTimer_toc");
        ...
        case /* hash32_fnv1a("hebi_charts_LoopTimer_ticToc") */ 578924568: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_LoopTimer_ticToc");
        ...
        case /* hash32_fnv1a("hebi_charts_LoopTimer_waitForNextTick") */ 2999084238: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_LoopTimer_waitForNextTick");
        ...
        case /* hash32_fnv1a("hebi_charts_LoopTimer_getSecondsToNextTick") */ 420776018: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_LoopTimer_getSecondsToNextTick");
        ...
        case /* hash32_fnv1a("hebi_charts_LoopTimer_getNanosToNextTick") */ 3013832472: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_LoopTimer_getNanosToNextTick");
        ...
        case /* hash32_fnv1a("hebi_charts_LoopTimer_timeSeconds") */ 2932397192: {
            ...
            checkArgCount(nrhs, 0, "hebi_charts_LoopTimer_timeSeconds");
        ...
        case /* hash32_fnv1a("hebi_charts_LoopTimer_timeNanos") */ 1429901862: {
            ...
            checkArgCount(nrhs, 0, "hebi_charts_LoopTimer_timeNanos");
        ...
        case /* hash32_fnv1a("hebi_charts_LoopTimer_sleepNanos") */ 2868392724: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_LoopTimer_sleepNanos");
            size_t nanos = getScalar<size_t>(prhs[1], "nanos");
        ...
        case /* hash32_fnv1a("hebi_charts_LoopTimer_parkNanos") */ 3410159229: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_LoopTimer_parkNanos");
            size_t nanos = getScalar<size_t>(prhs[1], "nanos");
        ...
        case /* hash32_fnv1a("hebi_charts_LoopTimer_waitNanos") */ 3978608554: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_LoopTimer_waitNanos");
            size_t nanos = getScalar<size_t>(prhs[1], "nanos");
        ...
        case /* hash32_fnv1a("hebi_charts_LoopTimer_spinNanos") */ 3435998397: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_LoopTimer_spinNanos");
            size_t nanos = getScalar<size_t>(prhs[1], "nanos");
        ...
        case /* hash32_fnv1a("hebi_charts_LoopTimer_release") */ 2762685731: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_LoopTimer_release");
        ...
        case /* hash32_fnv1a("hebi_charts_Object3d_isVisible") */ 4037753891: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Object3d_isVisible");
        ...
        case /* hash32_fnv1a("hebi_charts_Object3d_setVisible") */ 3013872277: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Object3d_setVisible");
            int visible = getScalar<int>(prhs[2], "visible");
        ...
        case /* hash32_fnv1a("hebi_charts_Object3d_setOrientation") */ 3092801013: {
            static auto hebi_charts_Object3d_setOrientation = libLoader.getFunc<void(*)(ErrorInfo*, void*, double, double, double, double)>("hebi_charts_Object3d_setOrientation");
            checkArgCount(nrhs, 5, "hebi_charts_Object3d_setOrientation");
            ErrorInfo _error = {};
            double qx = getScalar<double>(prhs[2], "qx");
            double qy = getScalar<double>(prhs[3], "qy");
            double qz = getScalar<double>(prhs[4], "qz");
            double qw = getScalar<double>(prhs[5], "qw");
            hebi_charts_Object3d_setOrientation(&_error, getPtr<void*>(prhs[1]), qx, qy, qz, qw);
            checkError(_error);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Object3d_setOrientationRPY") */ 1032601474: {
            static auto hebi_charts_Object3d_setOrientationRPY = libLoader.getFunc<void(*)(ErrorInfo*, void*, double, double, double)>("hebi_charts_Object3d_setOrientationRPY");
            checkArgCount(nrhs, 4, "hebi_charts_Object3d_setOrientationRPY");
            ErrorInfo _error = {};
            double roll = getScalar<double>(prhs[2], "roll");
            double pitch = getScalar<double>(prhs[3], "pitch");
            double yaw = getScalar<double>(prhs[4], "yaw");
            hebi_charts_Object3d_setOrientationRPY(&_error, getPtr<void*>(prhs[1]), roll, pitch, yaw);
            checkError(_error);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Object3d_setTranslation") */ 4253214088: {
            static auto hebi_charts_Object3d_setTranslation = libLoader.getFunc<void(*)(ErrorInfo*, void*, double, double, double)>("hebi_charts_Object3d_setTranslation");
            checkArgCount(nrhs, 4, "hebi_charts_Object3d_setTranslation");
            ErrorInfo _error = {};
            double x = getScalar<double>(prhs[2], "x");
            double y = getScalar<double>(prhs[3], "y");
            double z = getScalar<double>(prhs[4], "z");
            hebi_charts_Object3d_setTranslation(&_error, getPtr<void*>(prhs[1]), x, y, z);
            checkError(_error);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Object3d_setPose") */ 1159682568: {
            static auto hebi_charts_Object3d_setPose = libLoader.getFunc<void(*)(ErrorInfo*, void*, double, double, double, double, double, double, double)>("hebi_charts_Object3d_setPose");
            checkArgCount(nrhs, 8, "hebi_charts_Object3d_setPose");
            ErrorInfo _error = {};
            double x = getScalar<double>(prhs[2], "x");
            double y = getScalar<double>(prhs[3], "y");
            double z = getScalar<double>(prhs[4], "z");
            double qx = getScalar<double>(prhs[5], "qx");
            double qy = getScalar<double>(prhs[6], "qy");
            double qz = getScalar<double>(prhs[7], "qz");
            double qw = getScalar<double>(prhs[8], "qw");
            hebi_charts_Object3d_setPose(&_error, getPtr<void*>(prhs[1]), x, y, z, qx, qy, qz, qw);
            checkError(_error);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Object3d_setTransform4x4") */ 3412822525: {
            static auto hebi_charts_Object3d_setTransform4x4 = libLoader.getFunc<void(*)(ErrorInfo*, void*, Transform4x4)>("hebi_charts_Object3d_setTransform4x4");
            checkArgCount(nrhs, 2, "hebi_charts_Object3d_setTransform4x4");
            ErrorInfo _error = {};
            if (mxGetM(prhs[2]) != 4 || mxGetN(prhs[2]) != 4) {
                mexErrMsgIdAndTxt("hebi_charts:InvalidInput", "Expected a 4x4 matrix.");
            }
            MatlabArray<double, mxDOUBLE_CLASS> matrix_arr(prhs[2]);
            Transform4x4 matrix = {matrix_arr.ptr, 1};
            hebi_charts_Object3d_setTransform4x4(&_error, getPtr<void*>(prhs[1]), matrix);
            checkError(_error);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Object3d_release") */ 424151368: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Object3d_release");
        ...
        case /* hash32_fnv1a("hebi_charts_Frame_to_Object3d") */ 3969566570: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Frame_to_Object3d");
            void* _value = hebi_charts_Frame_to_Object3d(getPtr<void*>(prhs[1]));
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        ...
        case /* hash32_fnv1a("hebi_charts_Mesh_isCentered") */ 822326588: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Mesh_isCentered");
        ...
        case /* hash32_fnv1a("hebi_charts_Mesh_setCentered") */ 485890492: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Mesh_setCentered");
            int centered = getScalar<int>(prhs[2], "centered");
        ...
        case /* hash32_fnv1a("hebi_charts_Mesh_getScale") */ 3920716242: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Mesh_getScale");
        ...
        case /* hash32_fnv1a("hebi_charts_Mesh_setScale") */ 262304758: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Mesh_setScale");
            double scaleUnitsToMillimeters = getScalar<double>(prhs[2], "scaleUnitsToMillimeters");
        ...
        case /* hash32_fnv1a("hebi_charts_Mesh_getDisplayStyle") */ 975132563: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Mesh_getDisplayStyle");
        ...
        case /* hash32_fnv1a("hebi_charts_Mesh_setDisplayStyle") */ 3775830343: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Mesh_setDisplayStyle");
            int style = getScalar<int>(prhs[2], "style");
            hebi_charts_Mesh_setDisplayStyle(getPtr<void*>(prhs[1]), style);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Mesh_setMeshTransform4x4") */ 2180935893: {
            static auto hebi_charts_Mesh_setMeshTransform4x4 = libLoader.getFunc<void(*)(ErrorInfo*, void*, Transform4x4)>("hebi_charts_Mesh_setMeshTransform4x4");
            checkArgCount(nrhs, 2, "hebi_charts_Mesh_setMeshTransform4x4");
            ErrorInfo _error = {};
            if (mxGetM(prhs[2]) != 4 || mxGetN(prhs[2]) != 4) {
                mexErrMsgIdAndTxt("hebi_charts:InvalidInput", "Expected a 4x4 matrix.");
            }
            MatlabArray<double, mxDOUBLE_CLASS> matrix_arr(prhs[2]);
            Transform4x4 matrix = {matrix_arr.ptr, 1};
            hebi_charts_Mesh_setMeshTransform4x4(&_error, getPtr<void*>(prhs[1]), matrix);
            checkError(_error);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Mesh_setMeshPoseRPY") */ 3430244833: {
            static auto hebi_charts_Mesh_setMeshPoseRPY = libLoader.getFunc<void(*)(ErrorInfo*, void*, double, double, double, double, double, double)>("hebi_charts_Mesh_setMeshPoseRPY");
            checkArgCount(nrhs, 7, "hebi_charts_Mesh_setMeshPoseRPY");
            ErrorInfo _error = {};
            double x = getScalar<double>(prhs[2], "x");
            double y = getScalar<double>(prhs[3], "y");
            double z = getScalar<double>(prhs[4], "z");
            double roll = getScalar<double>(prhs[5], "roll");
            double pitch = getScalar<double>(prhs[6], "pitch");
            double yaw = getScalar<double>(prhs[7], "yaw");
            hebi_charts_Mesh_setMeshPoseRPY(&_error, getPtr<void*>(prhs[1]), x, y, z, roll, pitch, yaw);
            checkError(_error);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Mesh_to_Object3d") */ 398257298: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Mesh_to_Object3d");
            void* _value = hebi_charts_Mesh_to_Object3d(getPtr<void*>(prhs[1]));
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        ...
        case /* hash32_fnv1a("hebi_charts_Robot_getDisplayStyle") */ 163598878: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Robot_getDisplayStyle");
        ...
        case /* hash32_fnv1a("hebi_charts_Robot_setDisplayStyle") */ 3200811722: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Robot_setDisplayStyle");
            int style = getScalar<int>(prhs[2], "style");
        ...
        case /* hash32_fnv1a("hebi_charts_Robot_getDof") */ 3894024574: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Robot_getDof");
            size_t _value = hebi_charts_Robot_getDof(getPtr<void*>(prhs[1]));
            if (nlhs > 0) plhs[0] = createMexSize(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Robot_setPositions") */ 4060801135: {
            static auto hebi_charts_Robot_setPositions = libLoader.getFunc<void(*)(ErrorInfo*, void*, DoubleSpan)>("hebi_charts_Robot_setPositions");
            checkArgCount(nrhs, 2, "hebi_charts_Robot_setPositions");
            ErrorInfo _error = {};
            MatlabArray<double, mxDOUBLE_CLASS> positions_arr(prhs[2]);
            DoubleSpan positions = {positions_arr.ptr, mxGetNumberOfElements(prhs[2])};
            hebi_charts_Robot_setPositions(&_error, getPtr<void*>(prhs[1]), positions);
            checkError(_error);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Robot_to_Object3d") */ 1269492559: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Robot_to_Object3d");
            void* _value = hebi_charts_Robot_to_Object3d(getPtr<void*>(prhs[1]));
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        ...
        case /* hash32_fnv1a("hebi_charts_Series3d_getColor") */ 1267755258: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Series3d_getColor");
        ...
        case /* hash32_fnv1a("hebi_charts_Series3d_setColor") */ 677907614: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Series3d_setColor");
            int color = getScalar<int>(prhs[2], "color");
        ...
        case /* hash32_fnv1a("hebi_charts_Series3d_to_Object3d") */ 2939041005: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Series3d_to_Object3d");
            void* _value = hebi_charts_Series3d_to_Object3d(getPtr<void*>(prhs[1]));
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        ...
        case /* hash32_fnv1a("hebi_charts_Line3d_setMaxPointCount") */ 4037341455: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Line3d_setMaxPointCount");
            size_t count = getScalar<size_t>(prhs[2], "count");
        ...
        case /* hash32_fnv1a("hebi_charts_Line3d_clear") */ 815267325: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Line3d_clear");
            hebi_charts_Line3d_clear(getPtr<void*>(prhs[1]));
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Line3d_setData") */ 3313487720: {
            static auto hebi_charts_Line3d_setData = libLoader.getFunc<void(*)(void*, DoubleSpan, DoubleSpan, DoubleSpan)>("hebi_charts_Line3d_setData");
            checkArgCount(nrhs, 4, "hebi_charts_Line3d_setData");
            MatlabArray<double, mxDOUBLE_CLASS> x_arr(prhs[2]);
            DoubleSpan x = {x_arr.ptr, mxGetNumberOfElements(prhs[2])};
            MatlabArray<double, mxDOUBLE_CLASS> y_arr(prhs[3]);
            DoubleSpan y = {y_arr.ptr, mxGetNumberOfElements(prhs[3])};
            MatlabArray<double, mxDOUBLE_CLASS> z_arr(prhs[4]);
            DoubleSpan z = {z_arr.ptr, mxGetNumberOfElements(prhs[4])};
            hebi_charts_Line3d_setData(getPtr<void*>(prhs[1]), x, y, z);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Line3d_addPoints") */ 592149656: {
            static auto hebi_charts_Line3d_addPoints = libLoader.getFunc<void(*)(void*, DoubleSpan, DoubleSpan, DoubleSpan)>("hebi_charts_Line3d_addPoints");
            checkArgCount(nrhs, 4, "hebi_charts_Line3d_addPoints");
            MatlabArray<double, mxDOUBLE_CLASS> x_arr(prhs[2]);
            DoubleSpan x = {x_arr.ptr, mxGetNumberOfElements(prhs[2])};
            MatlabArray<double, mxDOUBLE_CLASS> y_arr(prhs[3]);
            DoubleSpan y = {y_arr.ptr, mxGetNumberOfElements(prhs[3])};
            MatlabArray<double, mxDOUBLE_CLASS> z_arr(prhs[4]);
            DoubleSpan z = {z_arr.ptr, mxGetNumberOfElements(prhs[4])};
            hebi_charts_Line3d_addPoints(getPtr<void*>(prhs[1]), x, y, z);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Line3d_addPoint") */ 3496792187: {
            ...
            checkArgCount(nrhs, 4, "hebi_charts_Line3d_addPoint");
            double x = getScalar<double>(prhs[2], "x");
            double y = getScalar<double>(prhs[3], "y");
            double z = getScalar<double>(prhs[4], "z");
        ...
        case /* hash32_fnv1a("hebi_charts_Line3d_to_Series3d") */ 1298098948: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Line3d_to_Series3d");
            void* _value = hebi_charts_Line3d_to_Series3d(getPtr<void*>(prhs[1]));
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        ...
        case /* hash32_fnv1a("hebi_charts_Points3d_getMarkerShape") */ 3408937892: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Points3d_getMarkerShape");
        ...
        case /* hash32_fnv1a("hebi_charts_Points3d_setMarkerShape") */ 3521551648: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Points3d_setMarkerShape");
            int shape = getScalar<int>(prhs[2], "shape");
        ...
        case /* hash32_fnv1a("hebi_charts_Points3d_getMarkerSize") */ 2476486298: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Points3d_getMarkerSize");
        ...
        case /* hash32_fnv1a("hebi_charts_Points3d_setMarkerSize") */ 1065476686: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Points3d_setMarkerSize");
            double value = getScalar<double>(prhs[2], "value");
        ...
        case /* hash32_fnv1a("hebi_charts_Points3d_isSelfIllumination") */ 210770462: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Points3d_isSelfIllumination");
        ...
        case /* hash32_fnv1a("hebi_charts_Points3d_setSelfIllumination") */ 512389092: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Points3d_setSelfIllumination");
            int value = getScalar<int>(prhs[2], "value");
        ...
        case /* hash32_fnv1a("hebi_charts_Points3d_isVertexSharing") */ 1374619971: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Points3d_isVertexSharing");
        ...
        case /* hash32_fnv1a("hebi_charts_Points3d_setVertexSharing") */ 1791901321: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Points3d_setVertexSharing");
            int value = getScalar<int>(prhs[2], "value");
        ...
        case /* hash32_fnv1a("hebi_charts_Points3d_setMaxPointCount") */ 247628292: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Points3d_setMaxPointCount");
            size_t count = getScalar<size_t>(prhs[2], "count");
        ...
        case /* hash32_fnv1a("hebi_charts_Points3d_clear") */ 1098647088: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Points3d_clear");
            hebi_charts_Points3d_clear(getPtr<void*>(prhs[1]));
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Points3d_setData") */ 2851140133: {
            static auto hebi_charts_Points3d_setData = libLoader.getFunc<void(*)(void*, DoubleSpan, DoubleSpan, DoubleSpan)>("hebi_charts_Points3d_setData");
            checkArgCount(nrhs, 4, "hebi_charts_Points3d_setData");
            MatlabArray<double, mxDOUBLE_CLASS> x_arr(prhs[2]);
            DoubleSpan x = {x_arr.ptr, mxGetNumberOfElements(prhs[2])};
            MatlabArray<double, mxDOUBLE_CLASS> y_arr(prhs[3]);
            DoubleSpan y = {y_arr.ptr, mxGetNumberOfElements(prhs[3])};
            MatlabArray<double, mxDOUBLE_CLASS> z_arr(prhs[4]);
            DoubleSpan z = {z_arr.ptr, mxGetNumberOfElements(prhs[4])};
            hebi_charts_Points3d_setData(getPtr<void*>(prhs[1]), x, y, z);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Points3d_addPoints") */ 1010583341: {
            static auto hebi_charts_Points3d_addPoints = libLoader.getFunc<void(*)(void*, DoubleSpan, DoubleSpan, DoubleSpan)>("hebi_charts_Points3d_addPoints");
            checkArgCount(nrhs, 4, "hebi_charts_Points3d_addPoints");
            MatlabArray<double, mxDOUBLE_CLASS> x_arr(prhs[2]);
            DoubleSpan x = {x_arr.ptr, mxGetNumberOfElements(prhs[2])};
            MatlabArray<double, mxDOUBLE_CLASS> y_arr(prhs[3]);
            DoubleSpan y = {y_arr.ptr, mxGetNumberOfElements(prhs[3])};
            MatlabArray<double, mxDOUBLE_CLASS> z_arr(prhs[4]);
            DoubleSpan z = {z_arr.ptr, mxGetNumberOfElements(prhs[4])};
            hebi_charts_Points3d_addPoints(getPtr<void*>(prhs[1]), x, y, z);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Points3d_addPoint") */ 735251532: {
            ...
            checkArgCount(nrhs, 4, "hebi_charts_Points3d_addPoint");
            double x = getScalar<double>(prhs[2], "x");
            double y = getScalar<double>(prhs[3], "y");
            double z = getScalar<double>(prhs[4], "z");
        ...
        case /* hash32_fnv1a("hebi_charts_Points3d_to_Series3d") */ 2229257233: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Points3d_to_Series3d");
            void* _value = hebi_charts_Points3d_to_Series3d(getPtr<void*>(prhs[1]));
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        ...
        case /* hash32_fnv1a("hebi_charts_RecordingResult_getDirectory") */ 1213405822: {
            static auto hebi_charts_RecordingResult_getDirectory = libLoader.getFunc<const char*(*)(ErrorInfo*, void*)>("hebi_charts_RecordingResult_getDirectory");
            checkArgCount(nrhs, 1, "hebi_charts_RecordingResult_getDirectory");
            ErrorInfo _error = {};
            const char* _value = hebi_charts_RecordingResult_getDirectory(&_error, getPtr<void*>(prhs[1]));
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexString(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_RecordingResult_getDroppedCount") */ 128995370: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_RecordingResult_getDroppedCount");
        ...
        case /* hash32_fnv1a("hebi_charts_RecordingResult_getDuration") */ 877501107: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_RecordingResult_getDuration");
        ...
        case /* hash32_fnv1a("hebi_charts_RecordingResult_getAverageFps") */ 1394133149: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_RecordingResult_getAverageFps");
        ...
        case /* hash32_fnv1a("hebi_charts_RecordingResult_getManifest") */ 2666175586: {
            static auto hebi_charts_RecordingResult_getManifest = libLoader.getFunc<const char*(*)(ErrorInfo*, void*)>("hebi_charts_RecordingResult_getManifest");
            checkArgCount(nrhs, 1, "hebi_charts_RecordingResult_getManifest");
            ErrorInfo _error = {};
            const char* _value = hebi_charts_RecordingResult_getManifest(&_error, getPtr<void*>(prhs[1]));
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexString(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_RecordingResult_getRecordedCount") */ 1112605622: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_RecordingResult_getRecordedCount");
        ...
        case /* hash32_fnv1a("hebi_charts_RecordingResult_getSkippedCount") */ 2669863444: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_RecordingResult_getSkippedCount");
        ...
        case /* hash32_fnv1a("hebi_charts_RecordingResult_getTotalFrames") */ 1060893205: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_RecordingResult_getTotalFrames");
        ...
        case /* hash32_fnv1a("hebi_charts_RecordingResult_getFfmpegCommand") */ 595525159: {
            static auto hebi_charts_RecordingResult_getFfmpegCommand = libLoader.getFunc<const char*(*)(ErrorInfo*, void*, int, int)>("hebi_charts_RecordingResult_getFfmpegCommand");
            checkArgCount(nrhs, 3, "hebi_charts_RecordingResult_getFfmpegCommand");
            ErrorInfo _error = {};
            int outputFormat = getScalar<int>(prhs[2], "outputFormat");
            int deleteDirectory = getScalar<int>(prhs[3], "deleteDirectory");
            const char* _value = hebi_charts_RecordingResult_getFfmpegCommand(&_error, getPtr<void*>(prhs[1]), outputFormat, deleteDirectory);
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexString(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_RecordingResult_runFfmpeg") */ 259333519: {
            static auto hebi_charts_RecordingResult_runFfmpeg = libLoader.getFunc<const char*(*)(ErrorInfo*, void*, int, int)>("hebi_charts_RecordingResult_runFfmpeg");
            checkArgCount(nrhs, 3, "hebi_charts_RecordingResult_runFfmpeg");
            ErrorInfo _error = {};
            int outputFormat = getScalar<int>(prhs[2], "outputFormat");
            int deleteDirectory = getScalar<int>(prhs[3], "deleteDirectory");
            const char* _value = hebi_charts_RecordingResult_runFfmpeg(&_error, getPtr<void*>(prhs[1]), outputFormat, deleteDirectory);
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexString(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_RecordingResult_release") */ 3748485838: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_RecordingResult_release");
        ...
        case /* hash32_fnv1a("hebi_charts_Scene3d_setGridBounds") */ 4249784247: {
            ...
            checkArgCount(nrhs, 7, "hebi_charts_Scene3d_setGridBounds");
            double minX = getScalar<double>(prhs[2], "minX");
            double maxX = getScalar<double>(prhs[3], "maxX");
            double minY = getScalar<double>(prhs[4], "minY");
            double maxY = getScalar<double>(prhs[5], "maxY");
            double minZ = getScalar<double>(prhs[6], "minZ");
            double maxZ = getScalar<double>(prhs[7], "maxZ");
        ...
        case /* hash32_fnv1a("hebi_charts_Scene3d_getMaxX") */ 646308900: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Scene3d_getMaxX");
        ...
        case /* hash32_fnv1a("hebi_charts_Scene3d_setMaxX") */ 3323998328: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Scene3d_setMaxX");
            double val = getScalar<double>(prhs[2], "val");
        ...
        case /* hash32_fnv1a("hebi_charts_Scene3d_getMaxY") */ 663086519: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Scene3d_getMaxY");
        ...
        case /* hash32_fnv1a("hebi_charts_Scene3d_setMaxY") */ 3340775947: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Scene3d_setMaxY");
            double val = getScalar<double>(prhs[2], "val");
        ...
        case /* hash32_fnv1a("hebi_charts_Scene3d_getMaxZ") */ 679864138: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Scene3d_getMaxZ");
        ...
        case /* hash32_fnv1a("hebi_charts_Scene3d_setMaxZ") */ 3357553566: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Scene3d_setMaxZ");
            double val = getScalar<double>(prhs[2], "val");
        ...
        case /* hash32_fnv1a("hebi_charts_Scene3d_getMinX") */ 3281704058: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Scene3d_getMinX");
        ...
        case /* hash32_fnv1a("hebi_charts_Scene3d_setMinX") */ 597161382: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Scene3d_setMinX");
            double val = getScalar<double>(prhs[2], "val");
        ...
        case /* hash32_fnv1a("hebi_charts_Scene3d_getMinY") */ 3298481677: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Scene3d_getMinY");
        ...
        case /* hash32_fnv1a("hebi_charts_Scene3d_setMinY") */ 613939001: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Scene3d_setMinY");
            double val = getScalar<double>(prhs[2], "val");
        ...
        case /* hash32_fnv1a("hebi_charts_Scene3d_getMinZ") */ 3248148820: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Scene3d_getMinZ");
        ...
        case /* hash32_fnv1a("hebi_charts_Scene3d_setMinZ") */ 563606144: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Scene3d_setMinZ");
            double val = getScalar<double>(prhs[2], "val");
        ...
        case /* hash32_fnv1a("hebi_charts_Scene3d_getGridStep") */ 1482289480: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Scene3d_getGridStep");
        ...
        case /* hash32_fnv1a("hebi_charts_Scene3d_setGridStep") */ 1127495300: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Scene3d_setGridStep");
            double gridStep = getScalar<double>(prhs[2], "gridStep");
        ...
        case /* hash32_fnv1a("hebi_charts_Scene3d_getCamera") */ 4036660369: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Scene3d_getCamera");
            void* _value = hebi_charts_Scene3d_getCamera(getPtr<void*>(prhs[1]));
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Scene3d_addRobot") */ 1709942591: {
            static auto hebi_charts_Scene3d_addRobot = libLoader.getFunc<void*(*)(ErrorInfo*, void*, const char*)>("hebi_charts_Scene3d_addRobot");
            checkArgCount(nrhs, 2, "hebi_charts_Scene3d_addRobot");
            ErrorInfo _error = {};
            MatlabString pathOrUrl(prhs[2]);
            void* _value = hebi_charts_Scene3d_addRobot(&_error, getPtr<void*>(prhs[1]), pathOrUrl);
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Scene3d_addMesh") */ 3080073628: {
            static auto hebi_charts_Scene3d_addMesh = libLoader.getFunc<void*(*)(ErrorInfo*, void*, const char*)>("hebi_charts_Scene3d_addMesh");
            checkArgCount(nrhs, 2, "hebi_charts_Scene3d_addMesh");
            ErrorInfo _error = {};
            MatlabString pathOrUrl(prhs[2]);
            void* _value = hebi_charts_Scene3d_addMesh(&_error, getPtr<void*>(prhs[1]), pathOrUrl);
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Scene3d_addFrame") */ 1393531910: {
            static auto hebi_charts_Scene3d_addFrame = libLoader.getFunc<void*(*)(ErrorInfo*, void*, double)>("hebi_charts_Scene3d_addFrame");
            checkArgCount(nrhs, 2, "hebi_charts_Scene3d_addFrame");
            ErrorInfo _error = {};
            double lengthInMeters = getScalar<double>(prhs[2], "lengthInMeters");
            void* _value = hebi_charts_Scene3d_addFrame(&_error, getPtr<void*>(prhs[1]), lengthInMeters);
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Scene3d_addLine") */ 1203584519: {
            static auto hebi_charts_Scene3d_addLine = libLoader.getFunc<void*(*)(ErrorInfo*, void*)>("hebi_charts_Scene3d_addLine");
            checkArgCount(nrhs, 1, "hebi_charts_Scene3d_addLine");
            ErrorInfo _error = {};
            void* _value = hebi_charts_Scene3d_addLine(&_error, getPtr<void*>(prhs[1]));
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Scene3d_addPoints") */ 4072579238: {
            static auto hebi_charts_Scene3d_addPoints = libLoader.getFunc<void*(*)(ErrorInfo*, void*)>("hebi_charts_Scene3d_addPoints");
            checkArgCount(nrhs, 1, "hebi_charts_Scene3d_addPoints");
            ErrorInfo _error = {};
            void* _value = hebi_charts_Scene3d_addPoints(&_error, getPtr<void*>(prhs[1]));
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Scene3d_release") */ 1418222175: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Scene3d_release");
        ...
        case /* hash32_fnv1a("hebi_charts_StreamView_release") */ 2205044721: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_StreamView_release");
        ...
        case /* hash32_fnv1a("hebi_charts_XYChart_getTitle") */ 446598326: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_XYChart_getTitle");
        ...
        case /* hash32_fnv1a("hebi_charts_XYChart_setTitle") */ 3453143722: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_XYChart_setTitle");
            MatlabString title(prhs[2]);
        ...
        case /* hash32_fnv1a("hebi_charts_XYChart_getXAssumeSorted") */ 3851556259: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_XYChart_getXAssumeSorted");
        ...
        case /* hash32_fnv1a("hebi_charts_XYChart_setXAssumeSorted") */ 3077688367: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_XYChart_setXAssumeSorted");
            int xAssumeSorted = getScalar<int>(prhs[2], "xAssumeSorted");
        ...
        case /* hash32_fnv1a("hebi_charts_XYChart_isXAutoUnitScaling") */ 790614670: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_XYChart_isXAutoUnitScaling");
        ...
        case /* hash32_fnv1a("hebi_charts_XYChart_setXAutoUnitScaling") */ 1247121678: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_XYChart_setXAutoUnitScaling");
            int enabled = getScalar<int>(prhs[2], "enabled");
        ...
        case /* hash32_fnv1a("hebi_charts_XYChart_getXLabel") */ 1702966006: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_XYChart_getXLabel");
        ...
        case /* hash32_fnv1a("hebi_charts_XYChart_setXLabel") */ 931614378: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_XYChart_setXLabel");
            MatlabString label(prhs[2]);
            hebi_charts_XYChart_setXLabel(getPtr<void*>(prhs[1]), label);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_XYChart_setXLim") */ 3399138658: {
            static auto hebi_charts_XYChart_setXLim = libLoader.getFunc<void(*)(ErrorInfo*, void*, double, double)>("hebi_charts_XYChart_setXLim");
            checkArgCount(nrhs, 3, "hebi_charts_XYChart_setXLim");
            ErrorInfo _error = {};
            double min = getScalar<double>(prhs[2], "min");
            double max = getScalar<double>(prhs[3], "max");
            hebi_charts_XYChart_setXLim(&_error, getPtr<void*>(prhs[1]), min, max);
            checkError(_error);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_XYChart_getXMax") */ 1111650342: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_XYChart_getXMax");
        ...
        case /* hash32_fnv1a("hebi_charts_XYChart_setXMax") */ 701119482: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_XYChart_setXMax");
            double max = getScalar<double>(prhs[2], "max");
        ...
        case /* hash32_fnv1a("hebi_charts_XYChart_getXMin") */ 875483748: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_XYChart_getXMin");
        ...
        case /* hash32_fnv1a("hebi_charts_XYChart_setXMin") */ 870175600: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_XYChart_setXMin");
            double min = getScalar<double>(prhs[2], "min");
        ...
        case /* hash32_fnv1a("hebi_charts_XYChart_getXUnit") */ 2514604662: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_XYChart_getXUnit");
        ...
        case /* hash32_fnv1a("hebi_charts_XYChart_setXUnit") */ 2396226850: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_XYChart_setXUnit");
        ...
        case /* hash32_fnv1a("hebi_charts_XYChart_isYAutoUnitScaling") */ 704255987: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_XYChart_isYAutoUnitScaling");
        ...
        case /* hash32_fnv1a("hebi_charts_XYChart_setYAutoUnitScaling") */ 1160762995: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_XYChart_setYAutoUnitScaling");
            int enabled = getScalar<int>(prhs[2], "enabled");
        ...
        case /* hash32_fnv1a("hebi_charts_XYChart_getYLabel") */ 3186521171: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_XYChart_getYLabel");
        ...
        case /* hash32_fnv1a("hebi_charts_XYChart_setYLabel") */ 1546141303: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_XYChart_setYLabel");
            MatlabString label(prhs[2]);
        ...
        case /* hash32_fnv1a("hebi_charts_XYChart_setYLim") */ 232695975: {
            ...
            checkArgCount(nrhs, 3, "hebi_charts_XYChart_setYLim");
            double min = getScalar<double>(prhs[2], "min");
            double max = getScalar<double>(prhs[3], "max");
        ...
        case /* hash32_fnv1a("hebi_charts_XYChart_getYMax") */ 558185587: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_XYChart_getYMax");
        ...
        case /* hash32_fnv1a("hebi_charts_XYChart_setYMax") */ 2517709703: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_XYChart_setYMax");
            double max = getScalar<double>(prhs[2], "max");
        ...
        case /* hash32_fnv1a("hebi_charts_XYChart_getYMin") */ 928573133: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_XYChart_getYMin");
        ...
        case /* hash32_fnv1a("hebi_charts_XYChart_setYMin") */ 2753876297: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_XYChart_setYMin");
            double min = getScalar<double>(prhs[2], "min");
        ...
        case /* hash32_fnv1a("hebi_charts_XYChart_getYUnit") */ 4268819729: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_XYChart_getYUnit");
        ...
        case /* hash32_fnv1a("hebi_charts_XYChart_setYUnit") */ 3765687725: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_XYChart_setYUnit");
            MatlabString unit(prhs[2]);
        ...
        case /* hash32_fnv1a("hebi_charts_XYChart_addXCursor") */ 310287279: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_XYChart_addXCursor");
        ...
        case /* hash32_fnv1a("hebi_charts_XYChart_addYCursor") */ 2095989168: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_XYChart_addYCursor");
        ...
        case /* hash32_fnv1a("hebi_charts_XYChart_release") */ 2008978777: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_XYChart_release");
        ...
        case /* hash32_fnv1a("hebi_charts_LatencyChart_addTrace") */ 2363952669: {
            static auto hebi_charts_LatencyChart_addTrace = libLoader.getFunc<void*(*)(ErrorInfo*, void*, const char*)>("hebi_charts_LatencyChart_addTrace");
            checkArgCount(nrhs, 2, "hebi_charts_LatencyChart_addTrace");
            ErrorInfo _error = {};
            MatlabString name(prhs[2]);
            void* _value = hebi_charts_LatencyChart_addTrace(&_error, getPtr<void*>(prhs[1]), name);
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_LatencyChart_to_XYChart") */ 1611720866: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_LatencyChart_to_XYChart");
            void* _value = hebi_charts_LatencyChart_to_XYChart(getPtr<void*>(prhs[1]));
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        ...
        case /* hash32_fnv1a("hebi_charts_LineChart_addLine") */ 59856830: {
            static auto hebi_charts_LineChart_addLine = libLoader.getFunc<void*(*)(ErrorInfo*, void*, const char*)>("hebi_charts_LineChart_addLine");
            checkArgCount(nrhs, 2, "hebi_charts_LineChart_addLine");
            ErrorInfo _error = {};
            MatlabString label(prhs[2]);
            void* _value = hebi_charts_LineChart_addLine(&_error, getPtr<void*>(prhs[1]), label);
            checkError(_error);
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_LineChart_to_XYChart") */ 2806901250: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_LineChart_to_XYChart");
            void* _value = hebi_charts_LineChart_to_XYChart(getPtr<void*>(prhs[1]));
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        ...
        case /* hash32_fnv1a("hebi_charts_XYSeries_getColor") */ 1905988630: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_XYSeries_getColor");
        ...
        case /* hash32_fnv1a("hebi_charts_XYSeries_setColor") */ 2118705778: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_XYSeries_setColor");
            int color = getScalar<int>(prhs[2], "color");
        ...
        case /* hash32_fnv1a("hebi_charts_XYSeries_getLabel") */ 3419944895: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_XYSeries_getLabel");
        ...
        case /* hash32_fnv1a("hebi_charts_XYSeries_setLabel") */ 182381595: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_XYSeries_setLabel");
            MatlabString label(prhs[2]);
        ...
        case /* hash32_fnv1a("hebi_charts_XYSeries_getLineStyle") */ 3465012550: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_XYSeries_getLineStyle");
        ...
        case /* hash32_fnv1a("hebi_charts_XYSeries_setLineStyle") */ 826964842: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_XYSeries_setLineStyle");
            int lineStyle = getScalar<int>(prhs[2], "lineStyle");
        ...
        case /* hash32_fnv1a("hebi_charts_XYSeries_getLineWidth") */ 1097775599: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_XYSeries_getLineWidth");
        ...
        case /* hash32_fnv1a("hebi_charts_XYSeries_setLineWidth") */ 1783421363: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_XYSeries_setLineWidth");
            double lineWidth = getScalar<double>(prhs[2], "lineWidth");
        ...
        case /* hash32_fnv1a("hebi_charts_XYSeries_getMarkerSize") */ 2363327772: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_XYSeries_getMarkerSize");
        ...
        case /* hash32_fnv1a("hebi_charts_XYSeries_setMarkerSize") */ 3401588224: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_XYSeries_setMarkerSize");
            double markerSize = getScalar<double>(prhs[2], "markerSize");
        ...
        case /* hash32_fnv1a("hebi_charts_XYSeries_getMarkerType") */ 3085579277: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_XYSeries_getMarkerType");
        ...
        case /* hash32_fnv1a("hebi_charts_XYSeries_setMarkerType") */ 1292126297: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_XYSeries_setMarkerType");
            int markerType = getScalar<int>(prhs[2], "markerType");
        ...
        case /* hash32_fnv1a("hebi_charts_XYSeries_isShowInLegend") */ 2272441234: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_XYSeries_isShowInLegend");
        ...
        case /* hash32_fnv1a("hebi_charts_XYSeries_setShowInLegend") */ 678502520: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_XYSeries_setShowInLegend");
            int showInLegend = getScalar<int>(prhs[2], "showInLegend");
        ...
        case /* hash32_fnv1a("hebi_charts_XYSeries_isVisible") */ 4264825601: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_XYSeries_isVisible");
        ...
        case /* hash32_fnv1a("hebi_charts_XYSeries_setVisible") */ 226469463: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_XYSeries_setVisible");
            int visible = getScalar<int>(prhs[2], "visible");
        ...
        case /* hash32_fnv1a("hebi_charts_XYSeries_release") */ 3347007086: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_XYSeries_release");
        ...
        case /* hash32_fnv1a("hebi_charts_LatencyTrace_tic") */ 3312694110: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_LatencyTrace_tic");
        ...
        case /* hash32_fnv1a("hebi_charts_LatencyTrace_toc") */ 3447209252: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_LatencyTrace_toc");
        ...
        case /* hash32_fnv1a("hebi_charts_LatencyTrace_ticToc") */ 893458976: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_LatencyTrace_ticToc");
        ...
        case /* hash32_fnv1a("hebi_charts_LatencyTrace_record") */ 289077951: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_LatencyTrace_record");
            double value = getScalar<double>(prhs[2], "value");
        ...
        case /* hash32_fnv1a("hebi_charts_LatencyTrace_recordWithCount") */ 3013829598: {
            ...
            checkArgCount(nrhs, 3, "hebi_charts_LatencyTrace_recordWithCount");
            double value = getScalar<double>(prhs[2], "value");
            size_t count = getScalar<size_t>(prhs[3], "count");
        ...
        case /* hash32_fnv1a("hebi_charts_LatencyTrace_recordCompensated") */ 2071056952: {
            ...
            checkArgCount(nrhs, 3, "hebi_charts_LatencyTrace_recordCompensated");
            double value = getScalar<double>(prhs[2], "value");
            double expectedIntervalBetweenValueSamples = getScalar<double>(prhs[3], "expectedIntervalBetweenValueSamples");
        ...
        case /* hash32_fnv1a("hebi_charts_LatencyTrace_reset") */ 2834722861: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_LatencyTrace_reset");
        ...
        case /* hash32_fnv1a("hebi_charts_LatencyTrace_to_XYSeries") */ 2481743194: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_LatencyTrace_to_XYSeries");
            void* _value = hebi_charts_LatencyTrace_to_XYSeries(getPtr<void*>(prhs[1]));
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        ...
        case /* hash32_fnv1a("hebi_charts_Line_getMaxPointCount") */ 1615881806: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Line_getMaxPointCount");
        ...
        case /* hash32_fnv1a("hebi_charts_Line_setMaxPointCount") */ 3014365810: {
            ...
            checkArgCount(nrhs, 2, "hebi_charts_Line_setMaxPointCount");
            size_t count = getScalar<size_t>(prhs[2], "count");
        ...
        case /* hash32_fnv1a("hebi_charts_Line_clear") */ 672966850: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Line_clear");
            hebi_charts_Line_clear(getPtr<void*>(prhs[1]));
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Line_setData") */ 3640589355: {
            static auto hebi_charts_Line_setData = libLoader.getFunc<void(*)(void*, DoubleSpan, DoubleSpan)>("hebi_charts_Line_setData");
            checkArgCount(nrhs, 3, "hebi_charts_Line_setData");
            MatlabArray<double, mxDOUBLE_CLASS> x_arr(prhs[2]);
            DoubleSpan x = {x_arr.ptr, mxGetNumberOfElements(prhs[2])};
            MatlabArray<double, mxDOUBLE_CLASS> y_arr(prhs[3]);
            DoubleSpan y = {y_arr.ptr, mxGetNumberOfElements(prhs[3])};
            hebi_charts_Line_setData(getPtr<void*>(prhs[1]), x, y);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Line_addPoints") */ 3366173531: {
            static auto hebi_charts_Line_addPoints = libLoader.getFunc<void(*)(void*, DoubleSpan, DoubleSpan)>("hebi_charts_Line_addPoints");
            checkArgCount(nrhs, 3, "hebi_charts_Line_addPoints");
            MatlabArray<double, mxDOUBLE_CLASS> x_arr(prhs[2]);
            DoubleSpan x = {x_arr.ptr, mxGetNumberOfElements(prhs[2])};
            MatlabArray<double, mxDOUBLE_CLASS> y_arr(prhs[3]);
            DoubleSpan y = {y_arr.ptr, mxGetNumberOfElements(prhs[3])};
            hebi_charts_Line_addPoints(getPtr<void*>(prhs[1]), x, y);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Line_addPoint") */ 4217019498: {
            ...
            checkArgCount(nrhs, 3, "hebi_charts_Line_addPoint");
            double x = getScalar<double>(prhs[2], "x");
            double y = getScalar<double>(prhs[3], "y");
        ...
        case /* hash32_fnv1a("hebi_charts_Line_to_XYSeries") */ 1080509183: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Line_to_XYSeries");
            void* _value = hebi_charts_Line_to_XYSeries(getPtr<void*>(prhs[1]));
            if (nlhs > 0) plhs[0] = createMexPointer(_value);
            break;
        }
        ...
        case /* hash32_fnv1a("hebi_charts_Runtime_setOption") */ 4101297084: {
            static auto hebi_charts_Runtime_setOption = libLoader.getFunc<void(*)(ErrorInfo*, int, const char*)>("hebi_charts_Runtime_setOption");
            checkArgCount(nrhs, 2, "hebi_charts_Runtime_setOption");
            ErrorInfo _error = {};
            int option = getScalar<int>(prhs[1], "option");
            MatlabString value(prhs[2]);
            hebi_charts_Runtime_setOption(&_error, option, value);
            checkError(_error);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Runtime_setTheme") */ 823017098: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Runtime_setTheme");
            int theme = getScalar<int>(prhs[1], "theme");
        ...
        case /* hash32_fnv1a("hebi_charts_Runtime_setAutoCloseWindows") */ 168480543: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Runtime_setAutoCloseWindows");
            int autoClose = getScalar<int>(prhs[1], "autoClose");
            hebi_charts_Runtime_setAutoCloseWindows(autoClose);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Runtime_waitUntilWindowsClosed") */ 1596282055: {
            static auto hebi_charts_Runtime_waitUntilWindowsClosed = libLoader.getFunc<void(*)(ErrorInfo*)>("hebi_charts_Runtime_waitUntilWindowsClosed");
            checkArgCount(nrhs, 0, "hebi_charts_Runtime_waitUntilWindowsClosed");
            ErrorInfo _error = {};
            hebi_charts_Runtime_waitUntilWindowsClosed(&_error);
            checkError(_error);
            break;
        }
        case /* hash32_fnv1a("hebi_charts_Runtime_collect") */ 1426291779: {
            ...
            checkArgCount(nrhs, 0, "hebi_charts_Runtime_collect");
        ...
        case /* hash32_fnv1a("hebi_charts_Runtime_closeAll") */ 1568276924: {
            ...
            checkArgCount(nrhs, 0, "hebi_charts_Runtime_closeAll");
        ...
        case /* hash32_fnv1a("hebi_charts_Runtime_printThreadInfo") */ 33274370: {
            ...
            checkArgCount(nrhs, 1, "hebi_charts_Runtime_printThreadInfo");
