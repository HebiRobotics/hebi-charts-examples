...
https://hdrhistogram.github.io/HdrHistogram/[HdrHistogram] is a popular tool in the low-latency community for recording jitter. It uses an efficient high-dynamic-range storage format for recording latencies with long tails. We support both official storage formats: `.hgrm` for pre-processed histogram data, and `.hlog` for interleaved histograms over time.
...
hdrHistogram.save_as_hrgm("overhead_python.hgrm", 1e9)
----
...
hdrHistogram.saveAsHrgm('overhead_matlab.hgrm', 1e9)
----
...
int main() {
    ...
    hdrHistogram.saveAsHrgm("overhead.hgrm", 1e9);
}
----
